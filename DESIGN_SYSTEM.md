...
Верхняя кромка прокручиваемых поверхностей — намёк на объём, а не растворение.
Формула `alpha(t) = minAlpha + (1 − minAlpha) · smootherstep(t)`; `smootherstep`
даёт нулевые производные на концах, иначе на стыках видна полоса Маха.
Гамма-коррекции нет: рампа живёт в узкой полосе у полной непрозрачности, где она
только утягивала бы контент к выцветанию.

Рампа короткая и слабая, потому что портит реальный контент: `20px` и
непрозрачность `0.65` у кромки для ленты и развёрнутой карточки, `16px` и `0.70`
для боковой панели и поиска. Списки слабее — текст строки несёт меньше визуальной
массы, чем фотография. Значения подобраны по светлой теме как по худшему случаю:
там любое осветление видно сразу.
