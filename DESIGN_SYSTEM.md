...
- **Pill** (по умолчанию) — сочетание клавиш и подпись двумя заливками в одной
  рамке, собственная геометрия компонента.
- **Standard** — стандартный `Button` дизайн-системы размера `xs` несёт
  ...
  Высота кнопки — `h-5`, как у внутренней пули варианта Pill: оба представления
  стоят на одной базовой линии панели. Сочетание клавиш и подпись в покое
  вторичные (`text-muted-foreground`), сочетание выходит на `foreground` под
  указателем — оно справочное, пока к нему не тянутся.
