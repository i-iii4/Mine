...
переключает её на вариант `reference` — то же тело `bg-component-fill`, без
единого `hover`-правила. Пока вариант оставался `default`, обводка приходила из
него и появлялась при наведении прямо на пилюлю. Ответ на курсор обещает
