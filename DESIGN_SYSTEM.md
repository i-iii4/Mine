...
(`--sidebar-row-pad-x: 16px`). Зона строки растёт с `92` до `104`, минимальная
ширина панели — с `342` до `314` (см. «Строка боковой панели: три зоны»).
...
Поле кнопки — `отступ кнопки + 84 + зазор 8`. Основной вариант: `92`;
Alt 1 и Alt 2: `104`.
...
Минимум: `342` в основном варианте, `314` в Alt. **Это число обязано совпадать
...
(`SIDEBAR_ROW_ACTION_BUTTON_PX` = 84 для `Connect` / `Connected` /
`Disconnect`), одной константой на все места, где этот контрол рисуется. `ch`
