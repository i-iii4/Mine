...
`src/lib/bottomBarOverflow.ts`): записи скрываются целиком по решённому порядку
`Navigate` → `Switch collection` → `Command` → `Focus` → `New Collection` →
`Hide Sidebar`; esc-записи, `Find elements`, `Settings` и Syncing не уходят
никогда. Усечение записи запрещено.
