...
2. **Заученные команды** — `Settings`, `Hide Sidebar`, `New Collection`,
   `Find elements`, `Commands`. Доступны всегда, используются постоянно, их
   сочетания есть и в нативном меню; панель — не то место, где их узнают
   заново.
3. **Ситуативные команды** — `Command`, `Focus`. Существуют только в том
   состоянии, которое их предлагает, поэтому панель — единственное место, где
   их вообще видно.

Не уходят никогда только esc-записи: состояние без видимого выхода хуже
переполненной панели.
