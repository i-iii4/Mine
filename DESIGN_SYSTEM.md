...
Навигация появляется, когда есть что перебирать; `Focus` — когда есть
сфокусированный объект, который откроет Enter; `Close card` — только при
открытой карточке; `Select` — когда в ленте есть клавиатурный фокус, от
которого можно вести выделение.
...
`Select` (`⇧ ↕ ↔`) описывает удержание: Shift удерживает карточку, от которой
считается диапазон, стрелка ведёт фокус дальше, и выделяется всё, что оказалось
между ними. Одного нажатия, которое мог бы выполнить клик, у такого удержания
нет, поэтому запись справочная.
...
остановка при табуляции **и отклик на наведение**. Ответ на курсор обещает
