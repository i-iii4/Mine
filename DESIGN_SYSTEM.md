...
Верхняя кромка прокручиваемых поверхностей создаёт впечатление контента за
границей и генерируется формулой
`alpha(t) = minAlpha + (1 − minAlpha) · smootherstep(t)^1.8`. `smootherstep`
даёт нулевые производные на концах — иначе на стыках видна полоса Маха; степень
`1.8` компенсирует гамму восприятия.

Длина рампы зависит от масштаба контента: `56px` и остаток `0.06` для ленты и
развёрнутой карточки, `28px` и остаток `0.08` для боковой панели и поиска. Ниже
`24px` рампа читается как ореол, а не как градиент. Стопы генерируются примерно
по одному на `2.5px`.
