...
`--radius-card` и `--radius-media` — единственные входы скругления карточки и её
медиа. Настройка Appearance → `Card corners` переопределяет оба токена сразу на
корневом элементе: `Square` (по умолчанию, значение из таблицы выше), `2`, `4`,
`8`, `16` пикселей.

`Square` не выставляет `0px`, а снимает переопределение — значение по умолчанию
остаётся за таблицей токенов, в одном месте.

Набор шагов временный: значения подбираются на реальном контенте, лишние потом
убираются.
