...
`Select` (`⇧ ↕ ↔`) описывает удержание: Shift держит карточку, с которой начат
путь, стрелка ведёт фокус дальше, и обе оказываются выделены. Одного нажатия,
которое мог бы выполнить клик, у такого удержания нет, поэтому запись
справочная.
