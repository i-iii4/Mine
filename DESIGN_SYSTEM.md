...
| L0.5 | `--sidebar` | 0.995 | 0.155 | Заливка боковой панели целиком |
...
**L0.5 — полушаг сайдбара.** Боковая панель визуально собирается собственной
заливкой: ровно середина между фоном страницы и chrome, полкванта лестницы.
Полный шаг ставил бы панель на один уровень с chrome и floating containers и
съедал бы различие «shell vs контент»; полушаг отделяет панель от страницы, не
конкурируя с верхним меню. `bg-sidebar` одновременно перебазирует `--surface`
зоны, так что все заливки и ховеры внутри панели поднимаются от её поверхности
(категория 3), контракт — `src/lib/sidebarSurface.test.ts`.
