...
**Статус:** принцип принят, реализация — отдельный шаг (введение `--surface`
convention: зона объявляет свой фон, потомки считаются от него через
`color-mix(in oklch, …)`). До реализации кнопки используют текущие абсолютные
`--component-fill*` (см. «Заливки компонентов»).
...
| Над поверхностью | консистентный lift над фоном зоны | относительное (целевое) |
...
Текущая реализация — отдельный набор токенов для кнопок (значения ниже). По
архитектуре (категория 3, relative elevation) кнопки должны перейти на lift над
фоном зоны; до реализации используются эти абсолютные значения.

| Токен | Светлая (L) | Тёмная (L) | Назначение |
|---|---|---|---|
| `--component-fill` | 0.9702 | 0.27 | Фон Button (default, destructive). ActionButton внешняя пуля |
| `--component-fill-inner` | 0.94 | 0.32 | ActionButton внутренняя пуля |
| `--component-fill-hover` | 0.91 | 0.37 | Hover/selected ActionButton. Hover-обводка Button |

Тёмная тема — равномерный surface-ладдер (L0 0.14 … L4 0.26); кнопки приподняты над поверхностями.
