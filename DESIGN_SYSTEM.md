...
### Плотность — две независимые оси

Appearance даёт две настройки со ступенями `32` / `24` / `16`:

- **Edge spacing** (`--edge-rhythm`) — расстояние контента от краёв окна и от
  chrome: верхняя и нижняя панели, таблица боковой панели, боковой и верхний
  отступы ленты, колонки и верхний отступ развёрнутой карточки.
- **Card spacing** (`--card-gap`) — зазор между карточками ленты, и только он.

Оси разделены потому, что отвечают на разные вопросы. Edge rhythm — про рамку
вокруг интерфейса, card gap — про плотность самого контента. Желание сдвинуть
карточки плотнее не равно желанию ужать весь интерфейс.

Раньше обе величины были вшиты в дизайн-вариант `alt` одним набором констант, и
любая из них тянула за собой вторую. Теперь `alt` отвечает только за раскладку.

Значения публикуются как CSS-переменные на корне: таблицы стилей и произвольные
значения Tailwind читают их напрямую, лента получает числа через
`useEdgeDensity` и `useCardGap`, потому что геометрия masonry считается в JS.
