...
**Справочная запись — не кнопка.** `Navigate` и `Focus` называют то, что
пользователь делает на клавиатуре; нажать их мышью нельзя. У таких записей
`ActionButton` получает `readOnly`: снимаются роль кнопки и попадание в обход
табуляцией, иначе Tab упирался бы в мёртвую остановку. Внешне запись не
отличается от команды — она читается как та же пара «клавиша и название».
