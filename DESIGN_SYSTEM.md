...
открытой карточке стрелки не делают ничего — лента их не слушает, а карточка не
...
`Close card` — только при открытой карточке.
...
бы использовать. Запись появляется при открытой карточке и в ленте — только при
клавиатурном фокусе: это единственное условие, при котором лента отвечает на
`⌘K`. Выделение мышью меню не открывает, поэтому и записи при нём нет. У
