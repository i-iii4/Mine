...
Ширина рампы: `24px` (`EDGE_FADE_WIDTH`) для правой кромки и `12px`
(`TOP_FADE_WIDTH`) для верхней. Верхняя вдвое уже: правая кромка гасит целую
строку текста вбок и нуждается в запасе на читаемость, верхней достаточно
смягчить уход контента под chrome. Рампа нелинейна, девять пар
`alpha / progress`, где `progress` — доля пути от непрозрачного начала к
прозрачной кромке:

| alpha | 0.82 | 0.64 | 0.49 | 0.36 | 0.25 | 0.16 | 0.09 | 0.04 | 0.01 |
|---|---|---|---|---|---|---|---|---|---|
| progress | 0.14 | 0.24 | 0.33 | 0.45 | 0.57 | 0.69 | 0.81 | 0.9 | 0.97 |

Линейная альфа читается как жёсткая полоса: воспринимаемая непрозрачность падает
быстрее самого значения альфы.

Две кромки:

- **правая** — строки и превью боковой панели; `clearTailWidth` резервирует
  прозрачный хвост под колонку действий;
- **верхняя** — прокручиваемые поверхности (боковая панель, лента, развёрнутая
  карточка, поиск); зеркальная рампа без резервного хвоста.

Верхняя рампа не доходит до нуля: у самой кромки остаётся
`TOP_FADE_MIN_ALPHA = 0.12`. Полностью растворённый контент читается как
обрезанный, слабый остаток — как контент, уходящий за chrome. Правая кромка
по-прежнему уходит в полную прозрачность: она скрывает переполнение текста, а не
границу.

Верхнее растворение включается только при `scrollTop >= 1px` и только при
включённой настройке `mine.scrollEdgeFade` (по умолчанию выключена). В покое
первая строка полностью непрозрачна. Полный контракт:
