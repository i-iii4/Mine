...
- Список пространств перечитывается при каждом открытии selector, включая
  pointer-путь верхнего хрома и клавиатуру. Источник обновления — общее состояние
  открытия, а не отдельный обработчик клика; ответ закрытого меню игнорируется.
  Добавленная через клиппер папка появляется без перезапуска приложения.
- `Open app` из клиппера переключает приложение в выбранное native-пространство
  и открывает Everything. При холодном запуске запрос ждёт завершения startup;
  при работающем приложении используется тот же механизм выбора пространства.
  Без выбранной папки действие только запускает приложение.
