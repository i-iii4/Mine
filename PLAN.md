...
### Отмена поля `type` для карточек — план 27.08.2026 (принято, реализация не начата)

Решение 044, контракт в [SPEC_BLOCK.md](SPEC_BLOCK.md). Этапы: правило вывода
вида из содержимого (порог подписи 600 — допущение) → запись и чтение без
`type` для карточек → вычистка поля из файлов хранилища одной перезаписью →
клипер и native host перестают ставить тип → `CreateBlockParams`/IPC/iOS без
`block_type` → спеки и тесты. `type: channel` остаётся маркером коллекции.
