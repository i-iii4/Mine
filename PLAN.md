...
### Надёжность индекса и доставка обновлений

Контракт: [SPEC_SYSTEM_RELIABILITY.md](</Users/i_iii/Проекты/Личные проекты/local-arena/SPEC_SYSTEM_RELIABILITY.md>).

- [x] Подготовить подробные требования по трём направлениям и матрицу приёмки.
- [x] Установить источник несовместимой запущенной сборки и воспроизвести отказ отдельно от пользовательских данных. Старый обработчик клиппера поддерживал схему 3, а его база имела схему 4.
- [x] Реализовать совместимые поколения индекса и ограниченное восстановление без удаления чужих баз. Финальная проверка установленного приложения остаётся отдельным пунктом.
- [x] Включить реестр принципов, проверку неизменности исходников и запрет выпуска без связанных с артефактами доказательств.
- [x] Реализовать базовый протокол сохранения, устойчивые черновики и операции, управляемые установки обработчика и CLI. Приёмка сочетаний версий остаётся открытой.
- [x] Реализовать проверку подписанного обновления, атомарную замену приложения, сохранение прежнего пакета и явное восстановление через настройки.
- [x] Завершить общий регрессионный прогон, собрать приложение и проверить NSFV → Mine в интерфейсе. Установленная Mine показывает коллекции и содержимое основного пространства.
- [ ] Реализовать проверенную доставку и активацию приложения и расширения.
- [ ] Закрыть матрицу A01 по A24 на собранных и установленных компонентах, включая все штатно достижимые сочетания версий начиная с базового выпуска B0.

Публичный канал, ключ подписи и сертификат Apple не подготовлены. Прежний каталог локального расширения обновлён через общий установщик; его файлы совпали со сборкой, зарегистрированный обработчик вернул 31 коллекцию Mine. Загрузка новой версии действующим браузером не подтверждена. CLI имеет безопасный установщик разработчика, но не включён в публичный пакет приложения. Эти ограничения не закрываются модульными тестами.

### Восстановление частично извлечённых медиа X

Контракт: [SPEC_CLIPPER.md](/Users/i_iii/Проекты/Личные проекты/local-arena/SPEC_CLIPPER.md).

- [x] Сохранять принадлежность медиа посту и цитате при авторизованном восстановлении.
- [x] Получать недостающий ролик, даже когда прямая GIF-ссылка уже найдена.
- [x] Формировать тело и предпросмотр из одного результата, исключить общую обложку X из видеопревью.
- [x] Проверить одинаковый набор в предпросмотре и запросе сохранения; записать оба реальных MP4 в отдельное тестовое пространство и сверить контрольные суммы.
- [x] Собрать и установить расширение и Mine, проверить подпись и совпадение установленного содержимого со сборкой.
- [x] Проверить результат повторного сохранения пользователем: 25.09.2026 в пространстве Mine записаны оба MP4 исходного поста в правильном порядке; SHA-256 обоих совпадает с оригиналами.
- [ ] Подтвердить визуально оба предпросмотра в личной сессии браузера. Проверка сохранённых файлов этого не подтверждает.

### Совместимость с Obsidian и вспомогательная история переименований

[Согласованный контракт](</Users/i_iii/Проекты/Личные проекты/local-arena/docs/space-storage-requirements.md#совместимость-с-obsidian>): одна папка, одинаковые документы, вложения, свойства и связи в Mine и Obsidian. Реализация прошла полный регрессионный прогон; [фактические проверки Obsidian](</Users/i_iii/Проекты/Личные проекты/local-arena/docs/obsidian-compatibility-acceptance.md>) отделены от визуальной приёмки.

- [x] Общее ядро разрешения и формирования ссылок используется приложением и обоими режимами клиппера. Сохранены чтение Markdown, псевдонимы и якоря. Граф, обратные связи и переходы из статьи используют тот же поиск цели.
- [x] Новые имена с учётом расширения уникальны во всём пространстве. Существующие одноимённые документы и вложения не объединяются.
- [x] Действующие связи читаются без истории. Устаревшие Obsidian-пути исправляются только при единственной цели.
- [x] История используется для подтверждённого внешнего переименования и безопасного переноса прежних привязок в исходные ссылки. Действующий явный адрес имеет приоритет.
- [x] Добавлены проверки конкурентной замены документа до публикации исправления и во время отката. Конкурентная версия сохраняется, повторная обработка неизменённых документов не переписывает их.
- [x] Добавлены проверки удаления истории до перемещения, удаления производных кэшей, повреждённой истории и сохранения без обязательной регистрации в истории.
- [x] На временной общей папке проверены реальные цели Obsidian 1.13.7 и Mine, перемещения без истории и переименование через Obsidian. Визуальное совпадение интерфейсов этим не заявляется.
- [x] Полный прогон: 1045 тестов Rust, 1255 тестов интерфейса, 19 проверок Rust/WASM и тест расширения с двумя перезапусками браузера. Приложение собрано, подпись проверена, установленная копия совпадает со сборкой и запущена. Журнал подтверждает `first_cards_painted` и `interactive`. Комплект клиппера и нативный обработчик установлены и сверены со сборкой.

### Места создания новых файлов

[Требования к хранению пространства](</Users/i_iii/Проекты/Личные проекты/local-arena/docs/space-storage-requirements.md>): настройки записи и перемещения с сохранённым реестром проверены и установлены. Это не подтверждает выполнение уточнённого контракта совместимости выше.

- [x] Три независимых пути новых файлов, корень при пустом или отсутствующем поле, начальные папки нового пространства, запись без восстановления удалённой папки при чтении.
- [x] Настройки путей и общий контракт приложения, CLI, native host и автономного клиппера закреплены целевыми тестами.
- [x] Автоматически проверить сохранение связей после внешних перестановок, одноимённых файлов, закрытого приложения и пересоздания производного индекса. Прошли 1015 проверок Rust, 1250 проверок интерфейса, 19 проверок соответствия реализаций и 19 проверок расширения с перезапусками. Проверки типов и стиля прошли. Нативный захват регистрируется до подтверждения записи; автономный браузерный захват впервые регистрируется при нативном сканировании.
- [x] Собрать production версию и установить `/Applications/Mine.app`. Хэш установленного приложения совпал со сборкой, подпись проверена. Запущенный процесс дошёл до `first_route_committed`.
- [x] Установить файлы клиппера и нативный помощник штатным установщиком. Хэш установленного помощника совпал со сборкой; содержимое установленного расширения совпало со сборкой.
- [ ] Выполнить визуальную приёмку установленного приложения и проверить перезагрузку уже открытого браузерного расширения. Эти действия не подтверждены.

### Извлечение товарных страниц

Спецификация и последовательность работ: [извлечение товаров](/Users/i_iii/Проекты/Личные%20проекты/local-arena/SPEC_PRODUCT_EXTRACTION.md).

- [x] Зафиксировать подтверждённую диагностику, целевую архитектуру и границы изменений.
- [x] Локализовать потерю восьми изображений: правило Defuddle для классов с окончанием `:hidden`. Смешанные галереи остаются на прежнем обработчике.
- [x] Подготовить воспроизводящие и отрицательные тесты.
- [x] Реализовать изолированный обработчик перед общим парсером, без изменения старых обработчиков.
- [x] Проверить реальный HTML, контракт предпросмотра, итоговый Markdown ядра, регрессии и скорость. Загрузка медиа и открытый интерфейс не проверялись.
- [x] Собрать и сверить доставку: приложение и установленный комплект расширения совпадают со сборкой, подпись проверена.
- [ ] Подтвердить перезагрузку расширения и работу открытого браузерного интерфейса. Управление браузером не разрешено.
...
состояние: Alt 1 является единственной доступной раскладкой. Варианты
`Default` и `Alt 2` сохранены только в истории проекта. Действующее правило
описано в [DESIGN_SYSTEM.md](DESIGN_SYSTEM.md), раздел «Варианты оформления».
