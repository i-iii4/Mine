...
### Отмена типов и классов карточек — план 27.08.2026 (принято, реализация не начата)
...
перестают ставить тип → `CreateBlockParams`/IPC/iOS без `block_type` → спеки и
тесты. `type: channel` остаётся маркером коллекции.
