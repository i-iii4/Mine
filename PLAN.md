...
порогов; импорт Are.na проверен на живом аккаунте (О18).

Открыто на 15.08.2026:

- **`bun run test:graph` красный.** Разобрано 15.08.2026: граф рисуется
  правильно и раскладка устойчива, падает именно порог аудита. Гейт требует,
  чтобы закрашенная область занимала не меньше 45% канвы по обеим осям; по
  вертикали получается 94.9%, по горизонтали 43.8% — недобор 1.2 пункта.
  `zoomToFit` вписывает облако узлов по меньшему измерению, а облако сейчас
  примерно втрое уже, чем окно 1280×800 по пропорции. Диагностика в аудите
  вводит в заблуждение: она печатает `loadCount: 0` всегда, потому что счётчик
  ставится эффектом родителя, а загрузку запускает эффект потомка — потомок
  выполняется раньше. Воспроизводится на коммите, предшествующем всей фазе R.
  Нужно решить, изменились ли параметры сил или порог был откалиброван под
  другое облако; до этого `bun run verify` не пройдёт целиком.
- Импорт Are.na на живом аккаунте (О18) — не проверялся ни разу.
- Публикация расширения в Chrome Web Store — внешний процесс.
