...
- **`bun run test:graph` красный.** Граф не закрашивает канву; воспроизводится
  на коммите, предшествующем всей фазе R, то есть к ней не относится. Требует
  отдельного разбора — до него `bun run verify` не пройдёт целиком.
