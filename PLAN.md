...
### Пространства клиппера и Open app

- [x] Единое обновление списка пространств при открытии меню мышью и клавиатурой; защита от поздних ответов закрытого меню.
- [x] `Open app` передаёт выбранное пространство; приложение принимает зарегистрированный путь и открывает Everything через существующий механизм переключения.
- [x] Запрос сохраняется до готовности frontend; поддержаны запуск закрытого приложения и переключение работающего.
- [x] 97 тестов, TypeScript и целевой ESLint; release собран и установлен.
- [x] Реальный native-host: запуск и переключение проверены; подпись и установленные payload клиппера сверены со сборкой.

### Источник клипа соответствует сохранённому контенту

- [x] Общий `CaptureResult` для превью и Save; источник статьи связан с извлечённым контентом.
- [x] Пермалинки социальных постов вместо главной страницы из metadata; сохранены выбранное фото X и выделенный текст.
- [x] Защита от позднего ответа прежнего документа/режима; pinned operations и старые заметки не мигрируются.
- [x] 231 тест расширения, TypeScript, проверка JS и архив сборки.
- [x] Payload установлен в стабильную папку расширения; идентификатор и совпадение файлов проверены.
- Перезагрузка расширения в Dia не подтверждена: AppleEvent завершился тайм-аутом, Computer Use не разрешён. Пользователь принял этот объём завершения; браузер и его вкладки не перезапускались.
...
- [x] Пакетный `get_grid_rows`: общий read model ленты, SQLite snapshot, до 200 slug.
- [x] Очередь 32 ms, дедупликация, один запрос одновременно, отмена при смене маршрута.
- [x] Защита от чужого пространства и старых ответов; поздний полный snapshot не откатывает новое превью.
- [x] 173 frontend-теста и 13 Rust-тестов целевых модулей.
- [x] Release-сборка, установка в `/Applications/Mine.app`, проверка подписи и запуска.

### Единые области нажатия chrome

- [x] Общая область 30px, отдельная визуальная плашка без изменения иконок и горизонтальных полей.
- [x] Верхняя/вторая/нижняя строки; явный chrome-режим для переиспользуемых компонентов.
- [x] Browser-регрессия: меню Everything и логотипа на 3px от разделителя; верхняя полоса кнопки кликабельна.
- [x] 206 компонентных тестов, 20 browser cases, TypeScript/ESLint; release установлен, подпись/executable и запуск проверены.

### Удаление сирот без блокировки интерфейса

- [x] Delete/list — фоновые blocking workers с отдельной read-only БД, без глобального mutex на время I/O.
- [x] Один пакет Trash; пути проверены, дубликаты исключены.
- [x] Общая функция удаления больше не переходит к permanent delete при отказе Trash.
- [x] Частичный отказ: ошибка пользователю и повторная загрузка списка.
- [x] 40 Rust-тестов, 6 frontend-тестов, TypeScript и ESLint.
- [x] Release установлен и запущен; подпись и совпадение executable проверены.

### Логотип в chrome: размер и цвет

- [x] Внутренние поля SVG без изменения кнопки 24px и canvas 16px.
- [x] Цвет наследуется от общего Button во всех состояниях.
- [x] Browser gate: 20 сценариев; release установлен и запущен, подпись проверена.

### Скругление вложенного медиаблока

- [x] Общий внешний контур GraphicSurface для single media / social grid / article media.
- [x] Радиус из настройки карточки, без скругления внутренних стыков сетки.
- [x] Регрессии сеток 1–4 изображений и одиночного/сеточного видео.
- [x] Release установлен и запущен, подпись и совпадение executable проверены.

### Стабильная геометрия изображения в Detail

- [x] Размер области из метаданных источника, два слоя одной геометрии.
- [x] Подмена после decode, сохранение превью при ошибке, reset при смене source.
- [x] Legacy: первая доступная геометрия не меняется от позднего оригинала/метаданных.
- [x] Browser gate: шесть случаев, задержанный original, contain и resize.
- [x] Установить release и подтвердить запуск; подпись и executable проверены.

### Поля кнопки связи в Sidebar

- [x] Общая плашка Connect / Connected / Disconnect, поля 8px со всех сторон.
- [x] Ширина от ячейки; единая модель минимальной ширины CSS/JS во всех дизайнах.
- [x] Тесты Sidebar, collection picker и sidebar column model: 55 passed.
- [x] Browser geometry gate: 18 сценариев (три состояния × три дизайна × 1x/2x), поля 8px со всех сторон.
- [x] Release установлен и запущен; подпись и совпадение executable проверены.

### HTML-видео в клиппере — реализовано и установлено

Контракт: `SPEC_CLIPPER.md`, раздел HTML video normalization.

- [x] Зафиксировать единый формат video для preview/save и границы browser-only.
- [x] Реализовать общий normalizer ArticleData без site-specific веток.
- [x] Проверить HTML/source/code/URL cases, идемпотентность, native/browser save payload.
- [x] Проверить реальный objkt MP4 в disposable vault: native save, локальный wikilink,
  полное декодирование 68,4 секунды с запретом сетевых протоколов.
- [x] Собрать и доставить установленный clipper payload; проверить версии файлов.
- [ ] Проверить новую версию в уже открытой пользовательской Dia-сессии:
  автоматическая перезагрузка extension без Computer Use/CDP недоступна.
  Native UI playback не проверялся; offline decode не подменяет эту приёмку.
...
## Сборка авторских тредов X — реализация подготовлена к ручной проверке

Контракт и фактические границы: [SPEC_X_THREADS.md](SPEC_X_THREADS.md).
Реализован наблюдатель запросов X, структурированный отбор цепочки, загрузка
продолжений. Save сохраняет показанное превью без галочки и блокировки по полноте.
Последняя страница ранжированных ответов не доказывает глобальную полноту.
«Тред» определяется связями авторских ответов, не смысловой классификацией.

Проверка 13.09.2026: на публичных данных подтверждена цепочка из четырёх постов
и исключение комментариев. Пагинация FxTwitter вернула 404; источник с доказанной
полнотой не найден. Прямой авторизованный транспорт X ещё не проверен.
Подробные результаты и границы доказательств — в спецификации; этап не закрыт.

- [ ] Подтвердить доступный источник связей ответов и окончания загрузки; закрепить fixtures и лимиты.
- [x] Заменить отбор соседних DOM-постов отбором по автору и родителю ответа.
- [x] Собирать полный текст и вложения каждой части, обрабатывать pagination и отмену при уходе со страницы.
- [x] Сохранять показанное превью через существующую идемпотентную операцию Save; не выводить технические предупреждения и дополнительные подтверждения полноты.
- [ ] Пройти матрицу тестов, реальный контрольный тред и проверку установленного расширения.

Новые платные API/серверы/разрешения не считаются согласованными. Существующие
заметки не переписываются. Недоступность данных не маскируется DOM-эвристикой.

## Исправления — 13.09.2026

- Реализованы и установлены: обновление геометрии превью, сохранение размеров
  при смене коллекций, актуальное drag-превью и ограниченный повтор загрузки.
- Массовое удаление выполняется одной фоновой командой и транзакцией;
  сохранение медиа не требует повторных обходов всего пространства.
- Delete Text и Create Element используют диапазон исходного Markdown вместо
  первого совпадения. Меню удерживает снимок выделения; ошибки видны пользователю.
- Медиа-действия разрешают короткие имена независимо от превью и отклоняют
  неоднозначность. Connect в поиске прокручивается внутри модального окна.
- Проверки второго пакета: 181 frontend-тест, 70 целевых Rust-тестов,
  7 тестов упаковки, TypeScript, линтер и два браузерных сценария.
- Release установлен в `/Applications/Mine.app`, хеши приложения и клиппера
  проверены; первые карточки и интерактивность по журналу — 844 мс.
- Границы приёмки: меню проверены в отдельном Chromium, не в установленном
  WKWebView; реальное время массового удаления не перемерялось. Скачки ленты
  и визуальные дубли без воспроизведения не объявлены исправленными.

Подробности, тесты и результаты двух пакетов: [DEVLOG.md](DEVLOG.md),
записи 13.09.2026. Исторические статусы ручной приёмки ниже не закрываются
прохождением этих целевых проверок.

## Исправления — 12.09.2026

- Реализовано и покрыто целевыми тестами: соблюдение сохранённой раскладки
  при создании файлов и устранение возврата удалённых коллекций из дубликатов.
- Упрощены сообщения и повтор сохранения в клиппере без смены исходной операции.
- Release установлен в `/Applications/Mine.app`; целостность runtime проверена,
  первые карточки по журналу запуска — 904 мс.
- Осталось: ручная приёмка Merge/удаления в приложении; отдельный установленный
  CLI не обновлён. Перенос старых файлов не входит в работу.

Подробности и границы проверок: [DEVLOG.md](DEVLOG.md), запись 12.09.2026.
Статусы ручной приёмки архитектурного плана ниже не изменены.
...
### Общее ядро сохранения — реализация 31.08.2026

Статус направления: **CODE IMPLEMENTED / DIA NATIVE PATH ACCEPTED / SC6 OPEN**. Контракт:
...
**Перенос кода выполнен.** Общий Rust/WASM, оба исполнителя, настройка,
desktop/CLI/FFI и проверки реализованы; `bun run verify:release` проходит.
Автоматические тесты не
подменяют реальную приёмку папки и порядка установки в Dia/Chrome; поэтому
весь план пока не помечен DONE. [Отчёт](docs/save-core-acceptance.md).

Дополнительно 31.08.2026 прошла прямая нативная приёмка
`bun run test:native-capture`: три состояния индекса, реальные Markdown/PNG,
lookup/replay отдельными процессами и две пересборки scanner-проекции.
01.09.2026 dev-регистрация Dia переключена на стабильный origin
`eioalidaccoahofcggkbinalibpajokh`, установленный helper обновлён и совпадает
с release-бинарём. Уже установленное расширение перезагружено в Dia и через
живой overlay сохранило ссылку в настроенный Mine vault. Старый extension ID
оставлен выключенным, его storage не удалён. Автономная OS-folder матрица,
остальные типы материалов и второй браузер остаются открыты.

03.09.2026 устранена зависимость unpacked-регистрации от checkout: canonical
builder создаёт минимальный payload, `.app` и developer installer обновляют
его в `Application Support/com.mine.app/clipper/extension`, а настройки
показывают этот стабильный путь. Dia переключён на новую копию:
расширение сохранилось после полного перезапуска браузера, открыло UI
и получило доступ к пространству Mine.

Ход SC0: [пробы, результаты и незакрытые границы](docs/sc0-save-safety.md).

| Этап | Статус | Зависит от | Работа | Критерий завершения |
|---|---|---|---|---|
| SC0. Контракт и безопасность | CONTRACT DONE; B1–B3 OPEN | Отмашка получена | Инвентарь владельцев правил и действующих API; конкретные типы запроса/результата; точки commit, повтор и журнал; эксперименты конкурентной записи и остановки на одноразовых папках | Зафиксированы границы native/browser, каждое окно сбоя и восстановления, правила совместимости; явно решён конфликт строгого no-clobber с ограничениями File System Access. Если нужны изменения пользовательской модели — отдельное согласование до продолжения |
| SC1. Переносимое ядро | DONE | SC0 | Выделить `mine-core`: модель, Markdown, имена, коллекции, относительные пути, общий сценарий. Системные факты передавать входами. Подключить native/WASM-сборку и генерацию типов | Одни входы и эталоны исполняются native и настоящим WASM и дают одинаковые байты/решения/ошибки. В ядре нет Tauri, SQLite, файлового/сетевого доступа; старые Rust-правила удалены или делегируют ядру |
| SC2. Нативный исполнитель | DONE; crash bounds documented | SC1 | Перевести native host на общий сценарий; отделить протокол от IO; перенести подготовку медиа и восстановление; убрать обязательное открытие индекса до source commit | Сохраняются media/security-контракты и `pending_uploads_v1`; занятый путь не меняется; отсутствующий/занятый/несовместимый индекс не мешает записи исходников; потери ответа и сбои каждого шага проходят контракт SC0 |
| SC3. Браузерный исполнитель | CODE DONE; packaged worker/restart PASS; OS-folder acceptance OPEN | SC1, закрытый browser-контракт SC0 | Подключить WASM в worker; оставить в JS только платформенные операции; очередь сохранений, состояние восстановления, traversal вложенных папок и работа с разрешением | Реальные Chrome/Dia сохраняют канонические примеры без host; пройдены остановка worker, отзыв разрешения, внутренние гонки и тесты внешней конкуренции с честными гарантиями. Независимые JS-правила и сценарий удалены в этом срезе |
| SC4. Подключение и настройка | CODE DONE; Dia native connection PASS; remaining live matrix OPEN | SC2, SC3 | Развести связь/совместимость/папку; стабильные dev/production ID; первый запуск и восстановление регистрации; отдельный extension-origin setup; привязка исполнителя к конкретному пространству | Оба порядка установки работают без ручного ID; оверлей не зацикливает выбор; черновик переживает настройку; подтверждены та же папка и неизменность начатой операции; сбой Dia воспроизведён и проверено его исправление |
| SC5. Клиенты и мосты | DONE | SC2 | Перевести затронутые desktop-команды, существующий iOS/UniFFI и CLI/MCP на общих владельцев; убрать доменные решения из мостов; сохранить действующие API/guards | Совместимость типов проверяется сборкой; существующие чтения не пишут и не запускают reconcile/watcher; текущие мутации сохраняют ограничения и dry-run. Новые пользовательские команды и iOS/Safari-функции не добавлены |
| SC6. Сквозная приёмка | AUTOMATED DONE; Dia native link PASS; remaining manual matrix OPEN | SC4, SC5 | Выполнить матрицу ниже в Chrome/Dia с реально собранными extension/host/app; проверять исходные файлы и их отображение | Для каждой строки есть результат и воспроизводимое доказательство. Успех означает нужные файлы в выбранной папке и корректную карточку; ни двойных сохранений после потери ответа, ни подмены неизвестного исхода успехом |
| SC7. Завершение перехода | CLEANUP DONE; closure depends on SC6 | SC6 | Повторный инвентарь, удаление оставшихся переходных вызовов/переключателей, обновление сборки и документов; полный прогон после удаления | Один владелец каждого правила и сценария; нет старого запасного движка. `bun run verify:release` и новые проверки ядра/клиппера проходят; старые vault читаются без автоматической перезаписи; опубликован отчёт о приёмке |
...
- Отмашка получена; SC0 не даёт разрешения обходить следующие барьеры.
...
### Отмена типов и классов карточек — план 27.08.2026 (реализовано 27.08.2026, остаток: IPC/iOS)

Решение 044, контракт в [SPEC_BLOCK.md](SPEC_BLOCK.md). Этапы: решение
отображения «только изображение / с информацией» по телу за вычетом
медиа-вставок (порогов нет) → запись и чтение без `type` для карточек →
вычистка поля из файлов хранилища одной перезаписью → клипер и native host
перестают ставить тип — **сделано 27.08.2026**. Остаток: `block_type` в
IPC-типах и iOS-мосте (теперь производная проекция) — удаление отдельным
заходом с пересборкой iOS. `type: channel` остаётся маркером коллекции.

### Доступ AI к материалам — план 25.08.2026 (принято 27.08.2026, реализация начата)
...
card, `--json` со стабильным контрактом) → мутационный слой (безопасный
каркас 27.08.2026: типизированные операции, защита вставок, dry-run, страховая
копия, restore) → MCP-сервер как адаптер (этап 2, отдельная приёмка).
...
### Phase R — Rollout readiness [SPEC, 14.08.2026]

Контракты приняты в рабочей сессии 13–14.08.2026 на основе двух аудитов.
Реализация не начата. Порядок внутри фазы задан зависимостями, а не важностью.

| # | Слой | Контракт | Состав |
|---|---|---|---|
| R0 | Инструмент | [SPEC_VAULT_LIFECYCLE.md](SPEC_VAULT_LIFECYCLE.md) П24 | **DONE 15.08.2026.** Приёмка холодного открытия сравнивает имена коллекций, а не пути; дубли попадают в сводку диагностикой; контракт закреплён тестом с папочной раскладкой |
| R1 | Данные | [SPEC_VAULT_LIFECYCLE.md](SPEC_VAULT_LIFECYCLE.md) П19–П21 | **DONE 15.08.2026.** Оба пути коммита удаляют каталог сразу; фоновая уборка при открытии пространства сносит накопленное, не трогая восстановимое |
| R2 | Раскладка | [SPEC_VAULT_LIFECYCLE.md](SPEC_VAULT_LIFECYCLE.md) П1–П4 | **DONE 15.08.2026.** Раскладка хранится в пространстве, применяется при генерации слага, медиа именуется по карточке и кладётся в свою папку; раздел `Folders` в настройках плюс действие `Organize` |
...
Definition of Done фазы: каждое состояние из R8 видно в витрине без
воспроизведения условий; браузерные проверки ленты проходят без ослабления
порогов. Пункт про импорт Are.na снят: импорт отменён в этой версии (см.
«Состав версии», принят 16.08.2026).

### Состав версии — принят 16.08.2026

Полный список работ до выпуска. Ничего из него не переносится «в следующую
версию»: такой версии нет как плана, есть только этот список. Порядок — по
убыванию риска, утверждён вместе с составом.

| # | Работа | Контракт | Статус |
|---|---|---|---|
| V1 | Автономная запись расширением без установленного приложения | [SPEC_ONBOARDING.md](SPEC_ONBOARDING.md) О1–О4; [SPEC_SAVE_CORE.md](SPEC_SAVE_CORE.md) | **REOPENED 31.08.2026; DIA NATIVE PATH PASS 01.09.2026.** Цикл выбора папки, transport/native диагностика и расхождение формата исправлены; живое сохранение через установленный helper прошло. Автономная OS-folder часть SC6 остаётся открытой |
...
| V9 | Копия пространства отличается от переезда | [SPEC_VAULT_LIFECYCLE.md](SPEC_VAULT_LIFECYCLE.md) П22 | **DONE 17.08.2026.** owner-path в derived store, копии чеканится свой id, переезд наследует; закреплено тестами обеих веток |
| V10 | Прогон `verify:release` при закрытом приложении | — | **DONE 17.08.2026, повторно 18.08.2026 на текущем HEAD.** Код возврата 0: биндинги, линтер, сборка, 878 фронтенд- и 791 Rust-тест, шесть браузерных аудитов (32 проверки), нативный smoke упакованного приложения (`ok: true` через tauri-ipc) |
...
| V12 | Ручная приёмка на реальном хранилище | — | пользователь |

Импорт Are.na отменён в этой версии без обещаний на будущее: вход из
онбординга убран 16.08.2026, код диалога спит без точки входа.

### Оформление после закрытия состава — идёт с 18.08.2026

Работа над вариантами раскладки идёт поверх закрытого состава и в него не
входит: состав описывает функциональность, а это подбор компоновки. Текущее
состояние — три варианта (`Default`, `Alt 1`, `Alt 2`), правки идут в `Alt 2`.
Что меняет каждый вариант — [DESIGN_SYSTEM.md](DESIGN_SYSTEM.md), раздел
«Варианты оформления»; ход работ — в [DEVLOG.md](DEVLOG.md).

Закрыто ранее:

- **`bun run test:graph` — закрыто 16.08.2026.** Порог покрытия 45% был
  откалиброван под то, как одна фикстура легла один раз, и падал на визуально
  нормальном графе (43.8% по ширине). Решение согласовано с пользователем:
  порог качества раскладки удалён, вместо него детектор вырожденного случая —
  10% по обеим осям, ловит только «все узлы в одной точке» и не имеет мнения об
  эстетике. Остальные проверки аудита (закрашенность канвы, обе темы, пунктир
  связей, панель, кадры) не тронуты. Диагностика починена: счётчик загрузок
  ставился эффектом родителя после эффекта потомка и всегда печатал
  `loadCount: 0`; теперь установка ленивая в самом счётчике. После этого
  `bun run test:browser` зелёный целиком — все пять аудитов.
- Импорт Are.na (О17–О18) — отменён в этой версии 16.08.2026, вход убран.

### Pre-rollout research checkpoint — 12.08.2026

Подготовка к раскатке на реальных пользователей: два аудита с полным перебором
сценариев, приоритизированными рекомендациями и подтверждением каждого факта
...
- [AUDIT_VAULT_RESILIENCE.md](AUDIT_VAULT_RESILIENCE.md) — устойчивость
  хранилищ. P0: cold-space-audit несовместим с папочными коллекциями (path vs
  name); дубль имени коллекции в подпапках возрождает NSFV-класс
  недетерминированных мутаций; пропавшее хранилище тихо сбрасывает привязку.
  Также: все новые файлы пишутся в корень мимо раскладки Collections/Cards/
  Media; перенос .md между папками при закрытом приложении меняет идентичность
  карточки.

Реализация рекомендаций не начата — приоритизация за пользователем.
...
| 29.3 | Settings frontend | [x] | Второй Vite-entry `settings.html` + `src/settings/` (SettingsApp, Appearance/Graph/Spaces/Orphans), общий ChromeShell/ChromeRow (30px содержимое, отдельный Separator 1px), bg-chrome + traffic-light reserve, nav 176px bg-active |
...
| 29.7 | Общая геометрия хрома и меню логотипа | [x] | ChromeShell/ChromeRow для всех уровней и Settings; 30px свободной области, плашки 24px, отдельные линии 1px; постоянный логотип и общий реестр восьми разделов. 86 целевых тестов и 16 browser-сценариев, pixel-проверка отступов 3/3px. Release установлен; native visual/fullscreen приёмка отдельно |
