...
Медиа собирается из вложения и из вложения цитируемого поста — цитата несёт
своё медиа рядом с цитируемой записью. Изображения берутся в размере
`fullsize`. Видео Bluesky отдаёт плейлистом HLS, который нельзя ни сохранить
файлом, ни показать в превью, поэтому от видео сохраняется постер.
