...
«Show more» и оставляет в разметке только видимую половину — сохранённая
карточка обрывалась посреди слова. Синдикационный API отдаёт пост целиком, но
теряет разметку: ссылки становятся голыми адресами.

Поэтому текст выбирается по посту, а не по источнику
(`extension/lib/tweetTextCompletion.js`): выигрывает разметка страницы, если она
полна; API — только когда текст страницы оказался её усечённым началом. Сравнение
идёт по нормализованной форме — markdown-ссылки уплощены до подписи, пробелы
сжаты, — потому что дословно два источника не совпадают никогда. Ответ API,
не начинающийся с прочитанного начала, отвергается: это другой пост.

Полный текст запрашивается **для каждой части треда**, а не только для той, на
которую указывает адрес: длинной может быть любая.

## Разделитель частей треда

Части треда разделяются `***`, а не `---`. Три дефиса на отдельной строке —
разделитель фронтматтера, и тело, начинающееся с них, читается как второй блок
фронтматтера — и Obsidian, и нашим разбором. `***` даёт ту же горизонтальную
линию без двусмысленности.

## Ссылка карточки при сохранении фото (25.08.2026)

Карточка ссылается на публикацию, а не на файл: файл — то, что скачивается,
ссылка — то, куда возвращаются. Приоритет источников
(`extension/popup/lib/postSourceUrl.ts`):

1. **Пермалинк поста из DOM вокруг картинки** (`findPostUrlForImage` в
   `content.js`). У X — ссылка отметки времени внутри ближайшего `<article>`
   (переживает цитируемые твиты, которые вносят второй `/status/`-адрес в тот
   же контейнер); иначе — ближайший контейнер, все пост-ссылки которого
   совпадают. Лайтбоксы отдают ту же картинку под другим вариантом размера,
   поэтому поиск `<img>` умеет сопоставлять по стабильному идентификатору:
   `pbs.twimg.com/media/<id>` у X, `<did>/<cid>` у Bluesky.
2. **Адрес страницы, если она сама — пост.** X внутри лайтбокса переписывает
   адрес на `/status/<id>/photo/1`; суффикс и query отбрасываются, ссылка
   нормализуется до `https://x.com/<handle>/status/<id>`.
3. **Адрес файла** — прежнее поведение, когда публикацию восстановить не из
   чего.

Страница профиля и корень `bsky.app` публикацией не считаются: такой адрес не
попадает в карточку с приоритетом выше файла. То же правило применяется в
ветке лайтбокса `save-page`, которая раньше оставляла карточке адрес профиля.

Ограничение: связь «файл → пост» существует только в DOM в момент сохранения —
публичного API для неё нет ни у X, ни у Bluesky. Если разметка не дала
пермалинка, карточка получает то, что и раньше.
...
Авторизованный путь включается, когда публичный ответ не содержит видео,
а в конкретном посте есть плеер или хотя бы одно уже найденное видео.
Одна прямая GIF-ссылка не доказывает полноту списка: соседний ролик может
оставаться доступным только авторизованной сессии.

`ArticleData.twitterPosts` сохраняет структуру постов и цитат до завершения
разрешения медиа. `hydrateTwitterPosts` обрабатывает каждый пост по его ID,
сохраняет фото и недоступные в новом ответе известные вложения. Разные
качества одного ролика сопоставляются по идентификатору в CDN-пути.
Полный ответ задаёт порядок видео; частичный не переставляет известные
вложения. Повторы между разными постами не удаляются.

Тело Markdown собирается общим `MineXThread.compose`, а список предпросмотра
формируется из тех же обновлённых постов. Запросы выполняются последовательно,
повторный ID внутри одного извлечения использует тот же ответ. Ошибка
разрешения сохраняет уже полученное содержимое и не блокирует Save.
Полный массив `extended_entities.media`, где разрешены все элементы,
не запрашивается повторно. Повторное получение нужно для частичного или
полученного из DOM набора.

Для каждого ролика используется его собственный постер либо кадр из MP4.
Общая `og:image` страницы X не подставляется вместо видеокадра.

Проверка регрессии: [twitterMedia.test.ts](/Users/i_iii/Проекты/Личные проекты/local-arena/extension/popup/lib/twitterMedia.test.ts)
и [useClipperState.standalone.test.tsx](/Users/i_iii/Проекты/Личные проекты/local-arena/extension/popup/hooks/useClipperState.standalone.test.tsx)
покрывают неполный набор, отказ транспорта, цитаты, фото, варианты качества
и совпадение предпросмотра с запросом сохранения.
[x-media-save-smoke.ts](/Users/i_iii/Проекты/Личные проекты/local-arena/scripts/x-media-save-smoke.ts)
проверяет реальное скачивание и запись двух MP4 контрольного поста
`2103621844733714547` во временное пространство. Ответ авторизованного
источника в этом тесте подставной; доступ в личной сессии им не проверяется.

25.09.2026 пользователь повторно сохранил [исходную публикацию](https://x.com/GasprArt/status/2103621844733714547)
в пространство Mine. Проверены два локальных MP4 и порядок их ссылок в
сохранённой заметке. Размеры файлов: 533545 и 185612 байт. SHA-256 совпали
с оригиналами: `9110a0aed8849754f7195e1059eac9c9d521c85e730027d73dae0d07fcd8eb19`
и `347081a589f437e86208e9a739a63230c52ae01c48bfc6c05a325a8aea85a88c`.
Это подтверждает реальное сохранение обоих вложений. Визуальная проверка
предпросмотра в браузере остаётся отдельным незавершённым пунктом.

Историческая схема до исправления частичного набора:
...
При отсутствии постера используется попытка получить кадр из прямого MP4.
...
### Устойчивый черновик и базовый протокол

[draftStore.js](/Users/i_iii/Проекты/Личные проекты/local-arena/extension/lib/draftStore.js)
сохраняет черновик в `chrome.storage.local` через одного писателя фонового
расширения. Сохраняются текст, название, теги, выбранная папка, исполнитель,
порядок медиа, сведения статьи и байты скриншота. Редакция подтверждается чтением
после записи. Save доступен только после завершения восстановления черновика;
до отправки операции текущая редакция записывается устойчиво.

Очистка после успеха проверяет одновременно `draftId` и ожидаемую редакцию.
Успех источника фиксируется отдельным результатом операции до очистки.
Если другое окно сохранило новую редакцию или очистка отказала, новый черновик
остаётся, исходное сохранение считается успешным, повторная запись операции
не выполняется. Неизвестные схемы и повреждённые записи не перезаписываются.

[protocol.ts](/Users/i_iii/Проекты/Личные проекты/local-arena/extension/popup/lib/protocol.ts)
согласует базовый протокол `1` и обязательные возможности `save_operation_v1`
и `operation_lookup_v1`, независимо от версии приложения. Отсутствующие поля
старого отправителя означают прежний базовый контракт; явно неизвестный протокол
или обязательная возможность отклоняются до доступа к папке и журналу.
Ответ содержит исходный `operation_id`, `outcome:not_committed` и
`terminal_rejected:true`. И браузерный исполнитель, и native host проверяют
запрос до эффектов. Старый виджет подтверждает совместимость с новым фоновым
расширением; несовпадение идентификаторов сборки само по себе не запрещает
совместимый Save.
...
| OS | Path |
|---|---|
| macOS | `~/Library/Application Support/com.mine.app/clipper/managed-v1/packages/<package-id>/native-host` |

Новый исполнитель находится в неизменяемом пакете `managed-v1/packages`.
Его собственный путь определяет также неизменяемую копию `yt-dlp` для уже
запущенной операции. Прежние каталоги `clipper/native-host` и
`LocalArena/native-host` сохраняются для старой установки, но новая регистрация
на них не ссылается.
...
устанавливается фоновым обслуживанием. [clipper_setup.rs](/Users/i_iii/Проекты/Личные проекты/local-arena/src-tauri/src/commands/clipper_setup.rs)
готовит полный неизменяемый пакет, проверяет SHA-256 и размеры, синхронизирует
записи и захватывает межпроцессную блокировку. До регистрации копия исполнителя
отвечает на `--runtime-probe` за 5 секунд: проверяются версия, объявленный
`native_host_build_id` и базовый протокол. Этот ответ не заменяет связь браузера.

Журнал установки сохраняет этапы `Prepared`, `HostActivated`,
`ExtensionActivated`, `LegacyExtensionActivated`, `Registered` и `FilesVerified`. После прерывания следующая
установка продолжает проверенный пакет. Повреждённый незавершённый пакет
сохраняется для диагностики, а предыдущий проверенный пакет восстанавливается
без отката исходных документов или журналов сохранения. В macOS каталог
расширения меняется через атомарный обмен каталогов. Все ресурсы установки
вместе не объявляются одной атомарной транзакцией.

Сравнение SemVer запрещает автоматическое понижение версии. Разные публичные
пакеты одной версии отклоняются. Явный режим разработки разрешает замену
разных сборок одной версии и сохраняет старый неизменяемый пакет. Идентификатор
сборки доказывает различие содержимого, но не задаёт порядок версий и не является
...
Поддержание установленного пакета выполняется после первого кадра. Обычная
проверка читает маркер, метаданные и проверяет запуск неизменяемого исполнителя;
полный пересчёт SHA-256 выполняется при изменениях, отсутствии компонентов или
по истечении 7 суток. Полный контракт запуска:
[SPEC_STARTUP_PERFORMANCE.md](/Users/i_iii/Проекты/Личные проекты/local-arena/SPEC_STARTUP_PERFORMANCE.md).

### Имя новой карточки

Имя выбирается до записи и обязано быть свободным во **всех** местах, куда
ляжет: путь карточки (с папкой карточек) и путь медиафайла (с папкой медиа и
голым именем). Сравнивать голое имя со списком путей нельзя — совпадений не
будет никогда, и клипер возьмёт занятое имя, а потом упадёт на
`failed to create block file`. Правило и его история:
[SPEC_DOMAIN.md](SPEC_DOMAIN.md), `resolve_card_name_conflict`.

### Manifest (Chromium)

Файлы:

- Chrome: `~/Library/Application Support/Google/Chrome/NativeMessagingHosts/com.mine.clipper.v1.json`
- Dia: `~/Library/Application Support/Dia/User Data/NativeMessagingHosts/com.mine.clipper.v1.json`

Manifests используют один protocol name и установленный native-host binary.
Публичный ключ [manifest.json](/Users/i_iii/Проекты/Личные проекты/local-arena/extension/manifest.json) фиксирует development ID
...
Новый каталог расширения:
`~/Library/Application Support/com.mine.app/clipper/managed-v1/extension`.
`build:extension` создаёт минимальный пакет без исходников и тестов, а `.app`
поставляет его ресурсом. `clipper:install-host` делегирует установку тому же
Rust-алгоритму через явный режим разработки; JavaScript не записывает
пользовательские каталоги самостоятельно.

Существующую распакованную установку по пути `clipper/extension`
[clipper_setup.rs](/Users/i_iii/Проекты/Личные проекты/local-arena/src-tauri/src/commands/clipper_setup.rs)
обновляет на том же пути. Первое принятие разрешено только для распознанного
старого расширения Mine с MV3, фиксированным ID
`eioalidaccoahofcggkbinalibpajokh`, исполнителем `background.js` и именем
`com.localarena.clipper`. Новый пакет обязан сохранить ID и использовать
`com.mine.clipper.v1`. Чужие каталоги, символические ссылки и изменённое
содержимое не заменяются. Последующие обновления принятого каталога требуют
совпадения с отпечатком поколения в журнале.

До атомарного обмена полная проверенная копия прежнего содержимого сохраняется
в `clipper/retained/extension-<sha256>`. Журнал фиксирует исходный отпечаток;
повтор после прерывания принимает только исходное или уже опубликованное
содержимое и проверяет сохранённую копию. Повреждённый незавершённый пакет
возвращает прежнее расширение из проверенной копии. Прежний исполнитель,
регистрация `com.localarena.clipper`, неизменяемые пакеты и незавершённые
операции сохранения не изменяются этой миграцией. Новый установленный пакет
не понижается ради старого содержимого приложения.

Регистрацией загруженного расширения и перезапуском его исполнителя владеет
браузер. Запись новых файлов по прежнему пути не доказывает, что браузер уже
загрузил их: приёмка требует реального ответа нового исполнителя и успешного
чтения коллекций. `FilesVerified` не является такой проверкой. Старый
установщик всё ещё знает прежний путь и может заменить его при отдельном
запуске. Профиль и хранилища браузера алгоритм не редактирует; публичные ключи
и адрес канала доставки отсутствуют, UPD не объявляется завершённым.

```json
{
  "name": "com.mine.clipper.v1",
  "description": "Mine web clipper native messaging host",
  "path": "/Users/user/Library/Application Support/com.mine.app/clipper/managed-v1/packages/<package-id>/native-host",
  "type": "stdio",
  "allowed_origins": [
...
Загрузка коллекций имеет отдельные состояния ожидания, успешного результата и отказа. `No collections` показывается только после успешного ответа с пустым списком. Ошибка чтения индекса или соединения сохраняет предпросмотр и возможность сохранения, показывает причину отказа загрузки и кнопку повторного запроса. Ответ для прежнего пространства либо более раннего запроса не заменяет актуальный список.
