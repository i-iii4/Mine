...
iOS-приложение Local Arena — мобильный компаньон к десктопу. Просмотр, поиск, сохранение контента из Share Sheet. Данные синхронизируются через iCloud Drive + CloudKit push. Общее Rust-ядро с десктопом через UniFFI.
...
```
1. Пользователь нажимает Share → Local Arena
...
│   └── local_arena_core.swift     # Auto-generated by UniFFI
...
- Выделить `local_arena_lib` в workspace crate
