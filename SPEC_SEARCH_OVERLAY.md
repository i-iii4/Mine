...
Непустой query короче **2 нормализованных символов** — pending-состояние, а не
поиск. Overlay очищает локальную выдачу, не делает IPC, не показывает счётчик
и не показывает `No results`. Один символ в vault-wide body/hybrid search
слишком шумный: например `a` одновременно является английским stopword и
массовым prefix-токеном. Ложное `0` хуже, чем спокойная пустая поверхность до
следующего символа. Backend `SearchPlan` дублирует этот guard и не строит план
для stopword-filtered query короче 2 alphanumeric символов, чтобы случайный
IPC/старый UI путь не мог вернуть выдачу для `г` через transliteration alias
`g*`.
...
```
ввод → normalizeSurfaceSearchQuery → min 2 chars → debounce 100ms
...
- **Инвалидация по мутациям vault** — двухфазная:
  ...
  2. **Truth**: оверлей слушает `vault-refreshed` (App диспатчит после каждого
     свежего grid-снапшота — удаление, rename, расширение, watcher) и молча
...
- один нормализованный символ не вызывает IPC, не показывает счётчик и не
  показывает `No results`;
