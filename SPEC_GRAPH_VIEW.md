...
**Масштаб ограничен сверху** `GRAPH_MAX_ZOOM`. Выше него карточки уже у
потолка размера, и дальнейшее приближение только раздвигает их: прокрутка
уходит в пустое поле с одной картинкой.
...
Пока физика зависела от объекта, каждое переключение окна перезапускало
симуляцию — граф расширялся и снова собирался. Она зависит от состава снимка:
маршрут, число узлов и связей, их идентификаторы.
