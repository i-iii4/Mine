...
**Размер — функция масштаба и плотности раскладки.** Формула одна:
`масштаб × GRAPH_NODE_FILL_RATIO / √плотность`, ограниченная снизу базовым
размером, сверху потолком и расстоянием до ближайших соседей.

Она выведена из счёта узлов в кадре, который был предыдущей моделью: число
узлов в кадре есть плотность, умноженная на площадь кадра, а площадь кадра —
это окно, делённое на квадрат масштаба. Подстановка даёт прямую
пропорциональность масштабу с коэффициентом от плотности.

Разница практическая, а не косметическая. Счёт узлов — величина дискретная:
один узел вышел за край, и размер всех карточек прыгнул. Чтобы эти ступени не
читались, приходилось сглаживать сам размер, и сглаживание превращалось в
запаздывание за трекпадом. Плотность непрерывна и меняется только вместе с
раскладкой, поэтому жест отображается на размер напрямую и точно.

Прямая привязка `size = base × zoom` — без коэффициента от плотности — была
первой и неверной попыткой: расстояния между узлами умножаются на тот же
масштаб, и доля экрана под картинками не менялась вовсе.
...
**Сглаживается плотность, а не размер.** Порог, принимавший
измерение только при расхождении больше двенадцати процентов, отвечал на
неверный вопрос: проблема была не в том, менять ли размер, а в том, как. Он
превращал любое настоящее изменение в лестницу — при открытии коллекции это
были три-четыре видимых ступени.

Плотность меняется без участия руки — осела раскладка, разошлись узлы, — и
сглаживание с постоянной времени `GRAPH_NODE_SIZE_TIME_CONSTANT_MS` читается
как то, что граф подстраивается сам. Масштаб — противоположный случай: рука
ведёт, и всё, что встаёт между жестом и размером, ощущается запаздыванием.
Поэтому путь зума не сглаживается вовсе.

**Новая раскладка отменяет прежнее измерение.** Плотность предыдущего вида к
новой коллекции отношения не имеет, и её перенос давал ровно то, что видел
пользователь: сначала мелкие миниатюры, потом рост в несколько заходов.

Порядок такой. При смене состава снимка размер считается по **расчётной**
плотности: коллизия держит центры на `2 × CARD_COLLISION_RADIUS`, а отталкивание
разводит соседей чуть дальше — множитель `GRAPH_SETTLED_SPREAD`, измеренный на
эталонном графе как 50,7 единиц против номинальных 44. С ним оценка попадает в
пределы трёх процентов от последующего измерения; на одном номинальном радиусе
промах составлял пятнадцать.

Вместе с плотностью **обязан сбрасываться признак «уже измерено»**. Без него
новая раскладка наследует его от прежней, раннего измерения не происходит вовсе,
и размер держится на оценке до самой остановки движка — на эталонном графе это
четырнадцать секунд, после которых всё меняется одним скачком. Ровно это
пользователь и видел. Первое измерение
на медленном такте заменяет расчёт **без сглаживания**: прийти к верному
размеру — не переход. Все последующие измерения ждут остановки движка и должны
превысить `GRAPH_DENSITY_CHANGE_THRESHOLD`, иначе поток слегка различающихся
измерений оседающей раскладки давал бы серию переходов — те самые рывки.
Шаг не зависит от частоты кадров, иначе переход шёл бы быстрее на быстрой
машине. Анимация заканчивается, когда остаётся меньше
`GRAPH_NODE_SIZE_SETTLE_PX`.

Кадры для неё берутся принудительным включением перерисовки (`autoPauseRedraw`)
на время перехода: осевший Canvas кадров не производит, и анимация замерла бы
на середине. Перерисовка касается только отрисовки — симуляция и позиции не
затрагиваются, что сохраняет правило «зум — только камера».

**Пока идёт перетаскивание, размер не меняется.** Перетаскивание разогревает
симуляцию, и между движениями мыши движок успевает остановиться; каждая
остановка пересчитывала заполнение кадра, и всё содержимое экрана пульсировало
под рукой.

**Углы скругляются вместе с ростом карточки.** Радиус отсчитывается **от
нижней границы размера**, а не от нуля: карточка в 32 пикселя имеет строго
прямые углы. Доля от размера дала бы там 1,6 пикселя — на экране удвоенной
плотности это три физических, и выглядит как размытый край, а не как
скругление.

Дальше радиус растёт непрерывно (`GRAPH_CARD_RADIUS_RATIO` на каждый пиксель
роста) и упирается в радиус, который интерфейс использует в остальных местах
(`GRAPH_CARD_RADIUS_MAX_PX`): 0 при 32, 1,35 при 50, 2,1 при 60, 5 при потолке.
Ни появления, ни исчезновения скачком нет ни в одной точке диапазона — это
проверяется тестом, который проходит весь диапазон по пикселю и требует, чтобы
наибольший шаг оставался малым.

**Карточка получает обводку по наведению** — ту же, что пилюли коллекций:
`hoverOutline` толщиной в один пиксель по тому же скруглённому контуру. Ховер и
выбор дают одну подсветку; своей толщины у выбора нет.

**Уровень миниатюры выбирается по целевому размеру, а не по текущему.**
Анимация пересекает порог уровня по дороге, и выбор по текущему значению
переключал бы картинку туда-обратно прямо во время перехода.

**Предел масштаба — обращение той же формулы, а не отдельный расчёт.**
`zoomForNodeSize` отвечает на вопрос «при каком масштабе карточка достигнет
такого размера»; предел приближения — это ответ для потолка, с запасом
`GRAPH_ZOOM_HEADROOM` на возможность рассмотреть отдельную карточку и нижней
границей `GRAPH_MAX_ZOOM_FLOOR` для очень разреженного графа.

Отдельной формулы для предела быть не должно: изменение расчёта размера иначе
оставляет приближение останавливающимся там, где карточки перестали расти
раньше. Согласованность проверяется тестом — размер на пределе равен потолку.
