...
**Границы размера держатся ограничением масштаба, а не обрезанием.**
`graphZoomBounds` даёт пределы, при которых карточка ровно на своей границе;
дальше камера не отпускает. Обрезание размера — это то, что порождает
наложение: карточка перестаёт уменьшаться, а расстояния продолжают, и граф
смыкается в ковёр.

Цена названа сознательно: граф, чей охват при минимальном масштабе больше окна,
смотрится панорамированием, а не целиком. По нынешней раскладке это наступает
примерно за тысячей карточек.
