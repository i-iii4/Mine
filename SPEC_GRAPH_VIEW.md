...
**Перетаскивание разогревает симуляцию ровно один раз.** У осевшего графа силы
почти не действуют, поэтому потянутый узел уходит один: соседи остаются на
месте, связи растягиваются через весь экран. Один вызов `d3ReheatSimulation`
при первом движении это оживляет.

Только один. Библиотека сама держит движок на низкой энергии всё время
перетаскивания — `alphaTarget(0.3)` на старте, снятие в конце. Разогрев на
каждое движение мыши вкладывал полную энергию в граф на каждом из них, и
раскладка металась, как в трёхмерном редакторе.
