...
**Углы скругляются вместе с ростом карточки.** Радиус — доля от экранного
размера (`GRAPH_CARD_RADIUS_RATIO`), ограниченная сверху радиусом, который
интерфейс использует в остальных местах (`GRAPH_CARD_RADIUS_MAX_PX`). На
нижней границе в 32 пикселя это меньше двух пикселей — то есть ничего, как и
должно быть при мелком узле; у потолка угол заметно смягчён. Скругление
пропорционально, а не появляется на пороге, иначе оно возникало бы скачком
посреди анимации размера.
