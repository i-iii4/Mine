...
**Статус: решение принято 21.08.2026, не реализовано.** Действующий код держит
`CARD_THUMBNAIL_SIZE = 32` и делит его на текущий масштаб, поэтому узел всегда
занимает ровно 32 экранных пикселя, а приближение только раздвигает связи.

**Целевое поведение — насыщение.** Размер узла растёт вместе с масштабом и
упирается в потолок: `size = min(GRAPH_NODE_MAX_PX, GRAPH_NODE_BASE_PX × zoom)`.
Приближаясь, пользователь видит, как картинки укрупняются; после потолка
продолжает работать только расхождение.
...
единиц графа, что на экране даёт `44 × zoom` пикселей. Отсюда:

| Размер узла | Достигается при масштабе | Зазор между картинками |
|---|---|---|
| 60 px | 1,4 | 60% размера |
| 100 px | 2,3 | 20% |
| 120 px | 2,7 | ноль, касание |

120 px — это точка соприкосновения; 100 оставляет воздух.
