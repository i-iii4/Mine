...
26.09.2026 подготовлен [SPEC_SYSTEM_RELIABILITY.md](/Users/i_iii/Проекты/Личные проекты/local-arena/SPEC_SYSTEM_RELIABILITY.md): возобновлена работа над доставкой и совместимостью компонентов. Локальная установка компонентов, их базовый протокол и устойчивые черновики реализованы в коде и проверены целевыми тестами. Публичная публикация, действующая регистрация браузера и матрица разных выпущенных версий не проверены. Телеметрия не включается автоматически; исходные файлы не откатываются.
...
1. Build `.app`.
2. Включить Hardened Runtime и определить минимальные разрешения.
3. Подписать вложенные исполняемые файлы, вспомогательные программы и фреймворки.
4. Пересчитать манифест компонентов из подписанных байтов, затем подписать внешний пакет без `--deep`.
5. Собрать `.dmg`.
...
Манифест компонентов должен описывать окончательно подписанные байты.
Порядок выпуска: подписать вложенные исполняемые файлы, затем пересчитать
манифест из фактических ресурсов внутри `.app`, затем подписать внешний пакет
без `--deep`. После этого проверить подпись и повторно сверить хеши компонентов.
Последующая глубокая подпись меняет вложенные байты и делает прежний манифест
недействительным; его проверку нельзя обходить.

Локальная воспроизводимая команда: `bun run build:local-app`.
Она завершает сборку через [finalize-local-bundle.mjs](/Users/i_iii/Проекты/Личные проекты/local-arena/scripts/finalize-local-bundle.mjs).
Для уже собранного артефакта: `bun run finalize:local-app`. Подпись здесь
специально локальная ad-hoc, без сертификата Apple и без нотариального заверения.
Прямой `bunx tauri build` без финализации не подтверждает готовность локального
артефакта к установке. Публичный выпуск требует Developer ID, Hardened Runtime,
согласованных разрешений и проверок Apple по описанному выше контракту.
...
### D8.1. Локальное обновление компонентов

[clipper_setup.rs](/Users/i_iii/Проекты/Личные проекты/local-arena/src-tauri/src/commands/clipper_setup.rs)
реализует отдельный локальный контракт установки клипера: межпроцессную
блокировку, неизменяемые пакеты, проверку SHA-256, ограниченную 5 секундами
проверку запуска исполнителя и устойчивый журнал этапов. Запущенный исполнитель
использует собственную неизменяемую копию `yt-dlp`; новые регистрации указывают
на новый пакет. Старые пакеты и неизвестные журналы сохраняются.

После прерывания журнал продолжается, а повреждённый незавершённый пакет
допускает восстановление предыдущего проверенного пакета без отката источника.
Понижение версии запрещено. Разные публичные пакеты одной SemVer-версии
отклоняются; отдельный режим разработки допускает замену сборки той же версии.
Идентификатор сборки подтверждает содержание, а не подпись или порядок версий.

`FilesVerified` и успешный `--runtime-probe` не означают активное расширение.
Перенос уже загруженной распакованной установки со старого пути в новый
`managed-v1/extension` браузером не реализован автоматически и не проверен.
Матрица реально запущенных разных версий приложения, расширения и старого
виджета остаётся открытой частью приёмки
[SPEC_SYSTEM_RELIABILITY.md](/Users/i_iii/Проекты/Личные проекты/local-arena/SPEC_SYSTEM_RELIABILITY.md).

Публичная доставка не проверена: адрес канала обновлений, ключ updater и
сертификат Apple отсутствуют. Локальная установка не заменяет подпись,
нотариальное заверение, публикацию расширения и проверку действующего браузера.
Контракт UPD не объявляется завершённым.

### D8.2. Локальная установка CLI

[runtime_installation.rs](/Users/i_iii/Проекты/Личные проекты/local-arena/src-tauri/src/runtime_installation.rs)
задаёт общие для клипера и CLI правила SemVer, блокировку установки, сохранение
неизменяемых исполняемых файлов и проверку фактической сборки. `cli:install`
собирает только CLI и вызывает узкий Rust-адаптер; сборка расширения не требуется.
Разрешён явный режим разработки с заменой сборки той же версии, но не понижением
уже установленной более новой версии.

Единственная точка активации CLI: атомарная ссылка `~/.local/bin/mine` на
проверенный исполняемый файл в неизменяемом пакете. Пакет и каталоги
синхронизируются до подтверждения установки; результат проверяется повторным
чтением фактической ссылки и манифеста. При прерывании ссылка указывает либо на
прежний, либо на новый полный пакет. Старый пакет сохраняется. Чужой файл,
чужая ссылка, неизвестная схема или повреждённый пакет не заменяются.

CLI не включён в публичный пакет приложения и не подключён к его опубликованной
доставке. Локальный установщик не доказывает выполнение UPD01 и UPD08 для CLI.
