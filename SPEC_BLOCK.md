...
## Отмена поля `type` для карточек — принято 27.08.2026, реализация не начата

Решение серии вопросов пользователя: тип карточки — рудонт, вид выводится из
содержимого, поле дублирует его и иногда врёт (клипер ставил `article` любому
посту X). Контракт после реализации:

**Вид карточки выводится только из содержимого:**

1. страница коллекции — по `type: channel` (см. ниже) либо по папке
   `Collections/`;
2. есть медиа (`file:` или вложение в теле) и тело — короткая подпись
   (< 600 знаков; допущение, правится одним числом) → **медиа-карточка**;
   картинка/видео/файл различаются расширением;
3. тело длиннее — **статья** (медиа становятся иллюстрациями);
4. нет тела, нет медиа, есть `url:` → **ссылка**;
5. нет ничего → пустая статья.

Это чинит исходную жалобу: пост X с видео и строкой подписи — медиа-карточка,
а не «статья с мусором».
...
- вычищается из существующих файлов хранилища одной перезаписью в рамках
  реализации (контекст: личное хранилище, один пользователь — церемония
  добровольной миграции не нужна);
- `type: channel` — единственное живое значение, маркер страницы коллекции.
  Нужен, потому что пустая коллекция в плоском хранилище не распознаётся ничем
  другим.

**Следствия:** `BlockType` в коде сжимается до маркера коллекции; `block_type`
уходит из `CreateBlockParams`, IPC-типов и iOS-моста (там вид берётся из
производного `card_kind`); карточка «вообще без содержимого» с бывшим
`type: image` меняет вид с пустой медиа на пустую статью.
