...
## Кавычки во фронтматтере (27.08.2026)

`yaml_quote` берёт значение в кавычки, если оно **заканчивается двоеточием**.
Без этого `title: … garden:` читается как ключ без значения, и стандартный
разбор YAML отказывается от всего документа — сохранённая карточка поста,
чей текст вводит следующий («turning my notes into a garden:»), оказывалась
нечитаемой. Двоеточие внутри слова по-прежнему кавычек не требует: YAML ломается
только на разделительных формах — `": "` и двоеточии в конце.

## Отмена поля `type` и классификации карточек — принято 27.08.2026, реализовано 27.08.2026

Итог обсуждения 27.08.2026: уходят и поле `type`, и сами понятия «статья» /
«картинка» как классы записей. Никаких порогов длины. Остаётся одно решение —
**про отображение**, и один параметр, который его принимает.

**Принцип.** Карточка в ленте показывает только изображение или изображение с
дополнительной информацией. Решает содержимое тела за вычетом медиа-вставок:

- **тело за вычетом вставок `![[…]]` пусто** → в ленте только изображение;
  вставки — это само содержимое, а не информация о нём;
- **в теле есть что-то ещё** (текст, ссылки, цитаты) → карточка выводит
  дополнительную информацию;
- **медиа нет вовсе** → карточка показывает текст; голая ссылка без тела —
  информацию из фронтматтера, больше показывать нечего.

Проверочные случаи: пост с четырьмя картинками без текста — только
изображения; пост с видео и текстом треда — изображение плюс информация;
заметка без медиа — текст.

**Поле `type`:**

- не пишется при создании карточек (клипер, native host, DropZone, вставка,
  `create_block`);
- игнорируется при чтении карточек — мёртвый текст;
- вычищается из существующих файлов хранилища одной перезаписью в рамках
  реализации (контекст: личное хранилище, один пользователь);
- `type: channel` — единственное живое значение, маркер страницы коллекции:
  пустая коллекция в плоском хранилище не распознаётся ничем другим.

**Следствия:** `BlockType` в коде сжимается до маркера коллекции; `block_type`
уходит из `CreateBlockParams`, IPC-типов и iOS-моста; внутренний `CardKind`
перестаёт быть классификацией записи и сводится к решению отображения по
принципу выше; картинка/видео/файл для проигрывания различаются расширением.

**Статус реализации (27.08.2026).** Разбор читает только маркер `channel`;
сериализация пишет `type:` только для коллекций; `derive_block_type` и
`derive_card_kind` следуют принципу; пути превью гейтятся формой содержимого
(`has_body_media`), манифест галереи строится из тела и для медиа-карточек;
native host выводит тип из содержимого. Хранилища Mine и NSFV вычищены
(27.08.2026): 644 карточки переписаны без `type`, 38 страниц коллекций не тронуты,
один битый заголовок с двоеточием закавычен.
