...
### Имя новой карточки: resolve_card_name_conflict

Отдельная функция, и разделение принципиальное. Имя карточки голое
(`Inspora`), а занятые места хранятся как пути относительно корня хранилища и
несут настроенную папку (`Cards/Inspora`). Сравнение голого имени с набором
таких путей не находит ничего никогда.

Именно это ломало веб-клипер: повторный клип уже сохранённой страницы получал
занятое имя и падал на `failed to create block file`. В хранилище пользователя
549 записей из 572 лежали в `Cards/`, то есть проверка не срабатывала почти
никогда.

Проверять нужно **оба** места, куда ляжет имя:

- путь карточки — `new_card_slug(name)`, с папкой карточек;
- путь медиафайла — `new_media_stem(name)`, с папкой медиа и голым именем,
  потому что медиа не наследует папку карточки.

Свободным считается только имя, свободное в обоих. `resolve_slug_conflict`
остаётся для сравнения уже готовых slug друг с другом.
