...
Актуальные требования к сохранению связей при перестановке файлов:
[Требования к пространству и сохранению файлов](</Users/i_iii/Проекты/Личные проекты/local-arena/docs/space-storage-requirements.md>).
Они заменяют прежнее допущение, что внешнее переименование может оставлять
устаревшие ссылки.
...
## Целевой контракт совместимости

[Требования к совместимости с Obsidian](</Users/i_iii/Проекты/Личные проекты/local-arena/docs/space-storage-requirements.md#совместимость-с-obsidian>) имеют приоритет над описанной ниже текущей реализацией. Действующие связи восстанавливаются из документов без реестра. Mine записывает кратчайшие однозначные Obsidian-ссылки и обеспечивает уникальность новых имён во всём пространстве. Существующие одноимённые файлы сохраняются раздельно.

Реестр нужен только как история для распознавания внешнего переименования. Он не переопределяет ссылку, изменённую в Obsidian, и не является условием чтения или перемещения файлов с уникальными именами. Найденное новое имя записывается в сами ссылки. Оба приложения должны показывать одну и ту же цель.

Статус: общее ядро, вспомогательная история, безопасное исправление источников и адаптеры сохранения реализованы. Прошли 1045 тестов Rust и 1255 тестов интерфейса. Цели ссылок и перестановка без истории сверены с реальным Obsidian 1.13.7 на временном пространстве. [Протокол проверки](</Users/i_iii/Проекты/Личные проекты/local-arena/docs/obsidian-compatibility-acceptance.md>) отдельно фиксирует границы этого результата.

## Архитектура нового механизма

### Источники и производные данные

| Слой | Содержимое | Последствие удаления |
| --- | --- | --- |
| Документы и медиа | Текст, свойства, явные ссылки, содержимое вложений | Удаляется сам материал |
| Каталог текущих путей | Снимок подходящих файлов всего пространства | Перестраивается сканированием |
| SQLite и превью | Представления текущих документов | Перестраиваются из источников |
| История переименований | Прежние пути и подтверждение непрерывности физического файла | Теряется только возможность доказать прежнее имя, действующие ссылки читаются без истории |

Основной путь чтения: документ, разбор ссылки, каталог текущих файлов, выбранная цель. История переименований в эту цепочку не входит. Отсутствие или повреждение истории не блокирует обычное чтение и подтверждение сохранения.

### Общее ядро ссылок

Чистый модуль общего Rust-ядра получает перечень путей относительно корня и выполняет разбор адресов, разрешение цели и формирование кратчайшего однозначного адреса. Файловый доступ, время, SQLite и история в модуль не входят. Нативные потребители и WASM используют одни правила.

Разрешение возвращает `Resolved`, `Missing` или `Ambiguous`. Неподдерживаемый адрес не разрешается и возвращает `Missing`. Список файлов индексируется один раз на проход, а не сканируется для каждой ссылки.

Для Obsidian-ссылки сначала проверяется существующий явный адрес, затем однозначное совпадение по суффиксу пути. При устаревшем адресе допустимо единственное совпадение имени во всём пространстве. Несколько совпадений не разрешаются по близости или алфавиту. Для обычной Markdown-ссылки учитываются её синтаксис и относительная база; правила восстановления Obsidian-ссылок не применяются к ней неявно.

`resolve_strict` использует те же правила без восстановления устаревшей папки по имени. История вызывает его перед попыткой восстановления прежнего файла. Это сохраняет приоритет действующей ссылки и исключает отдельную трактовку корневых файлов или имён с точками в обработчике истории.

Формирование ссылки начинает с имени и добавляет родительские сегменты до однозначного разрешения в ту же цель. Для заметок допустимо опустить расширение Markdown; расширения вложений сохраняются. Псевдоним и якорь отделяются от адреса и сохраняются. Кодовые блоки и произвольный текст свойств не переписываются как ссылки.

### Запись и уникальность имён

Планировщик сохранения получает актуальный снимок всех путей, включая корень и вложенные папки. Он выбирает свободное имя во всём пространстве и окончательный адрес по настройке записи. Ссылки формируются после выбора окончательных имён. Повторное использование медиа не создаёт новую копию.

Уникальность относится к полному имени с расширением: `photo.md` и `photo.jpg` не конфликтуют. Два новых `photo.jpg` в разных папках конфликтуют. Для уже существующей коллекции повторное создание с тем же именем сохраняет прежнее правило отказа, а не создаёт вторую коллекцию с числовым суффиксом.

Адаптер публикует новый файл без перезаписи существующего. При конкурирующем появлении цели нельзя подтвердить запись поверх чужих данных. Восстановление операции с тем же идентификатором использует журнал и доказательства уже опубликованных файлов, а не создаёт вторую карточку. История переименований не является обязательным этапом перед успешной квитанцией.

### Обработка перемещений и переименований

1. Перечитать текущие источники и обновить каталог путей.
2. Разрешить действующие ссылки только по документам и каталогу. Валидная изменённая пользователем ссылка имеет приоритет над старой историей.
3. Для устаревшей однозначной Obsidian-ссылки подготовить минимальное исправление адреса в исходном документе.
4. Для отсутствующего старого имени использовать историю только при подтверждённом переименовании того же физического файла и неизменённой соответствующей ссылке.
5. Безопасно записать исправленные ссылки. Лишь после успешной записи обновить производные представления и историю обработки.

Удаление истории перед перемещением уникальных имён между папками не меняет результат шага 2. Удаление истории перед произвольным переименованием не даёт права угадывать прежний файл по одинаковым байтам.

### Параллельные изменения

Внутренняя блокировка Mine упорядочивает только собственные операции. Она не считается защитой от записи Obsidian. Исправление ссылок должно проверять исходную версию и сохранять фактически вытесненные конкурентные данные до подтверждения успешной записи. Конфликт не подтверждается как успешное исправление, а его данные остаются восстанавливаемыми.

Для macOS реализация проверяет атомарный обмен подготовленного файла и исходника с анализом вытесненной версии. Простая последовательность «прочитать, сравнить, заменить» недостаточна: между сравнением и заменой возможна чужая запись. Откат не должен уничтожать третью версию, появившуюся после обмена. Без участия второго редактора нельзя обещать сериализуемость всех записей; проверяется отсутствие тихой потери версий при собственном исправлении Mine.

Повторное наблюдение собственных изменений не вызывает нового переписывания неизменённых документов. Ошибка одного документа не прерывает чтение остальных.

### Переход и проверки

Старые подтверждённые соответствия используются только для переноса информации в однозначные ссылки при отсутствии противоречащей пользовательской правки. Миграция повторяема и не требует удаления исходного реестра. После перехода его удаление не меняет разрешение действующих ссылок.

Обязательные проверки: отсутствие и повреждение истории; удаление истории и индекса до перемещения; уникальные и совпадающие имена; кратчайшие различающие суффиксы; материал клиппера при закрытом приложении; новое имя через Finder с историей; новая пользовательская цель вместо старой; конкурентная запись на границе публикации; повторная обработка без переписывания; одинаковое разрешение ссылок в реальном Obsidian на временном пространстве.

## Разделение реализации

1. Агент общего ядра: индекс путей, разрешение и формирование ссылок, нативные адаптеры чтения.
2. Агент истории: переименования, переход, безопасное исправление документов, наблюдение и интеграционные проверки.
3. Агент сохранения: единые правила новых имён и ссылок во всех путях записи и обоих режимах клиппера.

Основной агент согласует интерфейсы до интеграции, проверяет сценарии целиком и отличает автоматические проверки от ручной приёмки. Агенты не создают дополнительных субагентов.

## Предыдущая реализация до перехода

Этот раздел описывает заменяемую реализацию, а не действующее правило чтения. Источник привязок находился в `.mine/file-identity.json` внутри пространства.
Он хранит записи файлов с `id`, текущим `path`, физическим ключом
`device/inode/created_ns`, размером и временем изменения. Привязки ссылок
хранят `source_id`, исходный текст ссылки, синтаксис и `target_id`. Журнал
незавершённых изменений хранит `pending_source_ids` и `pending_revisions`.

При нативном сохранении захваченного файла регистрация источника завершается в той же
восстанавливаемой операции до подтверждения успешной записи. Повтор операции
после прерывания восстанавливает регистрацию, поэтому открывать приложение для
первой регистрации нативного захвата не требуется. Автономный браузерный
клиппер не записывает физический ключ файла; для созданных им источников
первичная регистрация выполняется при первом нативном сканировании.

При первом чтении ссылка привязывается только к единственной подходящей цели.
Одинаковое имя в нескольких папках, одинаковый текст или одинаковые байты
файлов не разрешают выбрать одну из них. На том же томе перенос подтверждается
физическим ключом файла. Атомарная замена Markdown по прежнему пути сохраняет
идентичность только если прежний ключ не найден в другом месте пространства.
Позже появившаяся единственная цель может разрешить прежнюю неоднозначность.
Одинаковый исходный текст в frontmatter, wikilink и Markdown хранит отдельные
привязки по синтаксису. Если исходный текст в индексе противоречит файлу,
обновление ссылки не производится.

После подтверждённого переноса затронутые ссылки переписываются по точным
позициям в исходном Markdown. Для wikilink, Markdown-ссылки и ссылки в
frontmatter применяются соответствующие правила синтаксиса. Перемещение само
по себе не требует правки заголовка, текста карточки или иных пользовательских
свойств. SQLite и превью остаются производными.
Ошибка чтения одного документа оставляет его в очереди; остальные документы
продолжают обрабатываться.

Если файл перенесён до первой регистрации или перенос лишил его физического
ключа без переносимого маркера, одинаковые подходящие файлы не позволяют
доказать соответствие. Произвольное копирование между устройствами с удалением
исходника не гарантирует сохранение идентичности. Такая связь остаётся
неразрешённой. Автоматические проверки открытого и закрытого приложения,
одноимённых файлов и пересоздания индекса прошли. Ручная проверка интерфейса
установленной версии остаётся открытой. Сборка и установка приложения
подтверждены, но видимый результат не проверен.

## Историческая спецификация

Разделы ниже описывают прежнюю модель по имени файла и хэшу. Их допущения о
потере ссылок, выборе дублей по пути и отсутствии исходных метаданных отменены
текущим контрактом выше.
