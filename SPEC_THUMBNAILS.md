...
### Обновление уже показанной карточки при готовности превью

`thumb:updated` означает инвалидирование данных карточки, а не только URL
картинки. Производитель публикует событие после записи файлов и метаданных.
Лента объединяет события за 32 ms, удаляет повторяющиеся slug и читает только
затронутые загруженные карточки пакетами не более 200 через `get_grid_rows`.
Команда читает индекс без сканирования хранилища; возвращает путь пространства,
ревизию и тот же read model, что обычная лента, в одном SQLite snapshot.

Пакет заменяет только существующие строки, не меняя порядок, число страниц,
выделение и позицию прокрутки. Кэш маршрутов инвалидируется. Ответ из прежнего
пространства/маршрута отбрасывается; ответ старее принятой ревизии не применяется.
События во время запроса образуют следующий пакет. Нет проверок по типу карточки,
периодического polling и полной перезагрузки ленты ради превью.

Проверки: статья без картинки → готовое медиа; новые размеры; одинаковый slug
в разных пространствах; поздние ответы; дубликаты; массовая очередь; удалённая
карточка не возвращается; неизменённые строки сохраняют object identity.
...
**I10. Фон текстовой миниатюры принадлежит теме, а не файлу.** Текстовая
заглушка хранит чернила на прозрачном фоне и ничего больше. Подложку кладёт
поверхность вывода, потому что только она знает тему, а файл переживает её
смену. Инвариант держится **на каждом уровне**: `full`, `micro`, `zoom`. Уровень
без альфы — нарушение, а не «сжатая копия»: непрозрачный файл нельзя положить ни
на какой фон, и `dark:invert` перекрашивает запечённый фон вместе с чернилами.

Владельцы подложки, по одному на способ отрисовки:

| Способ | Владелец | Чем красит |
|---|---|---|
| DOM | `MicroPreviewThumbnail` (`src/components/MicroPreviewThumbnail.tsx`) | `bg-card` на обёртке |
| Canvas (граф) | `drawGraphCard` (`src/components/graph/canvas.ts`) | `textPreviewFill`, прочитанный из `--card` |

Потребители подложку не выбирают. Пока выбирали — разошлись: боковая панель
красила `bg-accent`, поиск и строки ссылок `bg-component-fill`, граф не красил
вовсе. Цвет один и тот же — `--card`: текстовая миниатюра читается как
маленькая карточка ленты, поэтому и фон у неё ленточный.

В DOM подложка лежит на обёртке, а не на самом `<img>`: `dark:invert` действует
на элемент, на котором стоит, и фон на изображении инвертировался бы вместе с
чернилами. На холсте по той же причине заливка идёт **до** `ctx.filter`.
...
**Статус: реализовано 21.08.2026.** Уровни пишет `generate_thumb_levels`
рядом с полной миниатюрой, на каждом пути её создания; `backfill_thumb_levels`
досоздаёт их для хранилищ, где миниатюры уже были. На хранилище пользователя
это 1152 файла `micro` весом 1,9 МБ и столько же `zoom` весом 13,9 МБ против
142,7 МБ полных.
...
**П5. Декодер выбирается по содержимому файла, а не по расширению.** Текстовые
заглушки — это PNG, записанные как `<slug>.jpg`. `image::open` берёт декодер по
расширению, отдавал такие файлы JPEG-декодеру и падал: 38 карточек остались без
обоих уровней и рисовались тёмными квадратами.
...
Исключение — аварийное: если уровня **нет вовсе**, а не «ещё грузится», граф
берёт полную миниатюру. Тёмный квадрат вместо картинки хуже, чем разовая
переплата за одну карточку.

**П6. Кодек выхода выбирается по содержимому, как и кодек входа.** Уровень
сохраняет прозрачность источника: есть в исходнике хотя бы один пиксель с
`alpha < 255` — уровень пишется PNG, иначе JPEG.

Правило дописывает П5 со стороны записи. П5 починил чтение — `image::open`
брал декодер по расширению и падал на текстовых PNG, названных `.jpg`. Запись
осталась с тем же дефектом с другого конца: `generate_thumbnail` звала
`to_rgb8()` и JPEG-энкодер для любого входа. Альфа схлопывалась в чистый чёрный
(`RGBA(0,0,0,0)` → `RGB(0,0,0)`), и `micro` с `zoom` текстовой карточки
приезжали **чёрным квадратом с текстом**, поверх которого никакая подложка уже
не видна. Полная миниатюра при этом оставалась правильной, поэтому лента
выглядела здоровой, а боковая панель и граф — нет.

Признак берётся по пикселям, а не по наличию канала: PNG-скриншот приходит в
`RGBA` с полностью непрозрачными пикселями, и уводить такие в PNG-ветку значит
раздувать уровни медиа-карточек без причины. Проход по 64×64 бесплатен.

Побочная выгода: JPEG на мелком тексте даёт кольцевые артефакты вокруг букв, а
PNG на плоской заливке ещё и легче.

**Долг, который правило не гасит.** Расширение файла по-прежнему врёт:
`.micro.jpg` может содержать PNG. Это тот же корень, из которого уже выросли П5
и П6. Третий выход был вопросом времени и случился 29.08.2026 сразу в трёх
местах — см. П7. Честное лечение имени остаётся тем же: имя по содержимому
(`.micro.png`), ценой либо перебора двух расширений в `thumbnailLevelUrl`, либо
хранения формата уровня в БД.

**П7. Формат файла ничего не значит. Смысл спрашивают у источника и у
манифеста.** Пока текстовые заглушки были единственными PNG в кэше, формат
работал дешёвым признаком «это текст» и «это недоделано». П6 разорвал эту связь
окончательно: прозрачная картинка — законный PNG. Каждое место, продолжавшее
читать формат как смысл, дало свой отказ (29.08.2026):

| Что спрашивали у формата | Чем это оборачивалось | Чем отвечать |
|---|---|---|
| «Нужен ли браузерный апгрейд» | Готовая картинка гонялась через белый холст и получала белую рамку | Декодируемостью источника |
| «Текстовая ли миниатюра» | Прозрачная картинка инвертировалась в тёмной теме | `preview_manifest.kind` |
| «Готово ли превью» | `failed / browser_decode_required` навсегда; грид прячет манифест такой строки, и лента рисует имя файла вместо картинки | Декодируемостью источника |
| «Каковы размеры артефакта» | `image::open` по расширению `.jpg` не читал PNG внутри | Определением формата по содержимому |

Формат остаётся допустимым ответом ровно в одном случае: строка без манифеста,
то есть наследие до введения манифеста. Везде остальном он запасной вариант, а
не источник истины.

**Следствие для будущих правок.** Принудительное преобразование в JPEG было
костылём, на который молча опирались два из перечисленных мест. Убирая лишнее
преобразование, надо сначала найти тех, кто им пользовался, — иначе снятие
костыля превращает скрытый дефект в отказ.
...
### Миграция уровней, записанных без альфы

`backfill_thumb_levels` пропускает slug, у которого `micro` уже есть
(`src-tauri/src/storage/thumbnails.rs:1072`). Правило П6 само по себе чинит
только то, что пишется впервые: уровни, уже лежащие на диске чёрными, останутся
такими навсегда. Нужен отдельный однократный проход.

Условие перегенерации проверяется по четырём байтам, без распаковки: **`full`
начинается с PNG-сигнатуры, а `micro` — нет**. Такой уровень записан старым
кодеком и подлежит замене вместе с `zoom`. Медиа-карточек условие не касается —
у них `full` и так JPEG.

Замер на хранилище пользователя (`cea57568`, 581 карточка): 44 текстовых блока,
их `micro` и `zoom` — непрозрачный JPEG с углами `RGB(0,0,0)` при прозрачном
`full`. То есть перегенерации подлежат 88 файлов из 1942.
