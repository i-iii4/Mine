...
### 010: Колонка остаётся резиновой, перекладка при ресайзе принимается

Ширина колонки выводится из ширины ленты, а высота карточки зависит от ширины:
в текстовых карточках при другой ширине иначе ложатся строки. Поэтому пока
тянут ползунок панели или рамку окна, кладка пересобирается на каждый пиксель и
карточки перескакивают между колонками — это видно как дрожание сетки.

Рассматривалась фиксированная ширина колонки: высоты перестали бы меняться,
перекладка исчезла бы совсем, менялось бы только число колонок. Отклонено
20.08.2026. Дрожание длится ровно те секунды, пока человек сам тянет край;
остаток от деления при фиксированной колонке пришлось бы отдавать в поле у
правого края или в раздувшиеся зазоры — постоянно, при любой ширине окна. Это
обмен постоянного качества на устранение временного неудобства.

Диагностика при этом решении: работы в кадре нет. Профиль веб-процесса и
процесса композиции во время перетаскивания показал простой, браузерный замер —
ни одного пропущенного кадра. Пересчёт координат стоит 0,014 мс, пересчёт всех
высот 0,55 мс на 571 карточку. Дрожит не отрисовка, а сама раскладка.

Если когда-нибудь понадобится сгладить: порог перерисовки (реагировать, когда
ширина колонки изменилась на несколько пикселей) уменьшает частоту перекладки,
ничего не меняя в покое. Это единственный вариант без постоянной цены.
