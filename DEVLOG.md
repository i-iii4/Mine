...
## 03.04.2026 — Три бага: пустой грид, rename, белая вспышка

### Goal
Устранить пустой экран при первом запуске, исправить потерю данных при переименовании канала, убрать белую вспышку до загрузки CSS.
...
**2. Переименование канала сбрасывалось (App.tsx, commands.ts)**
Две причины: (a) race condition — `navigate()` срабатывал до `loadData()`, redirect-эффект видел несогласованное состояние (новый URL + старые tags) и перенаправлял на "/"; (b) нормализация — фронтенд использовал сырой ввод пользователя для URL, а бэкенд нормализовал тег. Исправление: `suppressRedirectRef` блокирует redirect во время операции, `loadData()` выполняется до `navigate()`, нормализованный тег берётся из ответа `renameChannel` (типизирован как `ChannelDto`).

**3. Белая вспышка при запуске (index.html)**
...
- Race condition между React Router (`navigate`) и React state (`useState`) — типичная проблема. Ref-флаг (`suppressRedirectRef`) гарантирует атомарность с точки зрения побочных эффектов.
