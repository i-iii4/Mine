## 23.08.2026 — Shift выделяет диапазон, а не пройденный путь

Первая версия выделяла только карточки, по которым прошёл фокус. Нужен диапазон:
всё между якорем и курсором, включая то, через что фокус не проходил.

В masonry «между» пришлось выбирать. Диапазон по порядку в списке при четырёх
колонках выделял бы на одно нажатие вниз четыре карточки, три из которых в чужих
колонках, — навигация стрелками геометрическая, и выделение разошлось бы с
движением фокуса. Взят прямоугольник охвата: `slugsWithinSelectionSpan` натягивает
прямоугольник на карточки якоря и курсора и включает те, чей **центр** внутри.
Центр, а не пересечение, — иначе карточка, задетая краем, попадала бы в выделение.

Диапазон пересчитывается от якоря на каждом шаге, а не накапливается: возврат
курсора на якорь оставляет одну выделенную карточку. Выделенное до постановки
якоря сохраняется как база — Shift расширяет, а не затирает.

Якорь снимается всем, что меняет выделение помимо Shift-хода: Escape, смена
коллекции, открытие карточки, Shift+Enter, клик с модификатором, рамка.

Проверка: 945 тестов. Восемь юнит-тестов на сам охват — включая то, что карточка
вне прямоугольника не выделяется, что порядок в списке не влияет и что
неготовые карточки пропускаются, — и пять сценариев в ленте.

## 23.08.2026 — Shift+стрелка выделяет по пути

В ленте появилось расширение выделения с клавиатуры. Shift держит карточку, с
которой начат путь, стрелка ведёт фокус дальше — после одного нажатия выделены
две. Каждая следующая Shift+стрелка добавляет карточку, на которую переходит
фокус. Шаг назад по тому же пути снимает выделение с покидаемой карточки.

Путь хранится списком в ref (`keyboardSelectionWalkRef`), а не выводится из
диапазона индексов: в masonry линейный диапазон между якорем и текущей карточкой
захватывает соседей из других колонок, которых на пути не было.

Стрелка без Shift завершает путь, но не трогает выделение — следующая
Shift+стрелка начинает новый якорь и добавляет к уже выделенному.

В нижнюю панель добавлена запись `⇧ ↕ ↔` `Select`, доступная при клавиатурном
фокусе в ленте — по тому же признаку, что и `Command`, поэтому проп
переименован в `onKeyboardCommandsChange`.

Проверка: 935 тестов, из них три новых — на «одно нажатие даёт две карточки», на
шаг назад и на невмешательство стрелки без Shift.
