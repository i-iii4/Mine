...
## 11.08.2026 — Stacked Detail: зазор до metadata row из токена плотности

### Задача
В stacked-раскладке Detail (широкое медиа, metadata под контентом) расстояние
между картинкой и карточкой метаданных было захардкожено как `mt-8` (32px) и
совпадало с боковыми отступами только на плотности Spacing 32px; на 24/16 не
сжималось.

### Что сделано
- `mt-8` заменён на `mt-[var(--edge-rhythm,32px)]` у
  `data-detail-stacked-metadata-row` — зазор теперь живёт на том же токене
  `--edge-rhythm`, что и боковые insets сетки (см. `src/lib/density.ts`).
- Тест раскладки обновлён на новый класс; в `DESIGN_SYSTEM.md` и
  `SPEC_FRONTEND.md` stacked-контракт теперь называет токен вместо
  захардкоженных `2rem`.
...
### Задача
Два сообщения от пользователя: в развёрнутой карточке не отображается медиа
(в ленте отображается) и перестала работать кнопка `Copy Path`.

### Изображения — системная поломка, а не одна карточка
Диагностика показала: сломаны все 247 карточек с медиа. Заметка ссылается на
файл обсидиановским wikilink'ом — голым именем (`file: "[[name.jpg]]"`), в
индексе `media_file` хранится так же, а фронтенд строил адрес как
`vaultPath + "/" + имя`, то есть искал файл в корне хранилища. После миграции
в `Cards/`/`Media/` там лежат 6 файлов из 1095. Лента выжила, потому что
показывает миниатюру из кеша, чей путь Rust отдаёт уже разрешённым.

Фикс — в протоколе ассетов: не найдя файл по прямому пути, он ищет его по
имени внутри хранилища (`resolve_basename_under`), как Obsidian резолвит
wikilink и как уже делал `resolve_indexed_media` для превью. Одна точка чинит
развёрнутую карточку, полноэкранный просмотр, действия над медиа и инлайн-медиа
статей. Найденный путь проверяется тем же asset scope — fallback не расширяет
разрешённое.

### Copy Path — веб-API в десктопном приложении
Копирование было на `navigator.clipboard.writeText` без `.catch()`. WKWebView
отклоняет запись, когда документ не в фокусе, а меню Radix при выборе пункта
фокус забирает — отказ уходил в никуда. Гипотеза подтвердилась тестом на одно
нажатие: `Cmd+L` (клавиатурный обработчик, меню не закрывается) копировал, а
кнопка в меню — нет. Копирование перенесено в систему через
`tauri-plugin-clipboard-manager`; заменены все четыре места, механизм в
приложении теперь один.

### Уроки
- Расхождение моделей адресации («имя» против «путь от корня») невидимо, пока
  всё лежит в одной папке. Чинить его на фронтенде значило бы зашить раскладку
  папок в клиент и сломать снова при настраиваемых путях.
- Веб-API буфера обмена в desktop-обёртке — ловушка по устройству: вызов без
  обработки ошибки превращает отказ в тишину, и баг выглядит как мёртвая кнопка.

## 10.08.2026 — SIGABRT: гонка asset-протокола с WKWebView

### Задача
Приложение падало «каждый раз» при обычном использовании: четыре SIGABRT за
день с одинаковым стеком, включая release-сборку. Параллельная сессия
атрибутировала падение в ONNX Runtime (`ort`/`fastembed`) по стрипнутому
бинарнику — release-крэш с целыми символами показал, что это ошибка.

### Причина
`panic in a function that cannot unwind`: наш asset-протокол отвечал WKWebView
из tokio-воркера, а `WKURLSchemeTask` — состояние главного потока. Отмена
задачи (скролл виртуализированной ленты снимает `<img>`, навигация) приходит
на main thread и вклинивается между проверкой валидности задачи в wry и
вызовами `didReceiveResponse/didReceiveData/didFinish`; WebKit бросает
NSException, она всплывает Rust-паникой внутри extern "C" фрейма — такая
паника не разматывается, процесс абортится целиком. Известная незакрытая
гонка Tauri/wry: https://github.com/tauri-apps/tauri/issues/12338 (wry 0.54.4
не исправляет).

### Фикс
`asset_protocol::register` отправляет `responder.respond(...)` через
`run_on_main_thread`: блок ответа wry и отмена задачи сериализуются на одной
очереди, окно гонки исчезает по построению. Дополнительно release-профиль
переведён на `panic = "unwind"` как страховочный пояс — Rust-паника в фоновой
задаче больше не убивает всё приложение (сам по себе unwind гонку не лечит:
паника в extern "C" абортит при любой стратегии). Release с фиксом запущен со
stderr-ловушкой (`~/Library/Logs/mine-stderr.log`) — при любом следующем
падении останется текст паники.

### Урок
Крэш со стрипнутыми символами — не почва для атрибуции: «ближайшее совпадение»
указало на невиновную библиотеку, и фикс готовился не там. Release-сборка с
целыми символами дала точный фрейм за минуту.

## 10.08.2026 — Клиппер: каркас панели, статусы, Escape, пространства

### Задача
Аудит UI клиппера подтвердил три жалобы: легаси-подтверждение сохранения
(StatusBar с хардкод-зелёным вне дизайн-системы), кнопка Save за экраном без
возможности доскроллить, отсутствие Add space / Reveal in Finder. Плюс
Escape рвал иерархию слоёв, самодельный Play-бейдж дублировал системный,
скроллбар-хак по utility-классу задевал общий пикер, мёртвый PreviewCard.

### Что сделано
- Высотный каркас: overlay-панель `max-h-[calc(100vh-32px)]`, окно —
  `height:100%` цепочка; шапки и save-футер жёсткие, сжимаются только зоны
  просмотра — картинка масштабируется (`object-contain` в `min-h-24 shrink
  overflow-hidden` боксе), article preview ужимается на свой скролл,
  квантованный список пикера отдаёт строки до пола `136px`. Кнопки Crop
  Area/Retake жёсткие (`shrink-0`) — их исчезновение было главным дефектом
  первой (сжимаемой без правил) версии; промежуточная модель одной
  скролл-зоны отвергнута доводом пользователя: она увозила пикер и кнопки в
  скролл. Ниже суммы минимумов (~430px, за пределами реальных окон) панель
  скроллится целиком. Кнопка Save всегда на экране по построению.
  Урок процесса: реализацию начинать только после отмашки — промежуточная
  версия ушла в код до подтверждения модели.
- StatusBar удалён. Успех — состояние кнопки (`Saved`, автозакрытие), ошибка —
  строка `text-destructive` над кнопкой (`data-clipper-save-error`).
- Escape слоями: capture-хендлер клиппера уступает текстовым полям и открытым
  dropdown; CollectionPicker (оба варианта) на Escape сначала чистит непустой
  запрос. Работает и в приложении.
- Пространства: host-команды `pick_vault_folder` (osascript choose folder →
  атомарная запись `known_vaults` в общий config.json) и `reveal_vault`
  (только известные пути). В VaultSelect — pinned `Reveal in Finder` /
  `Add space` с иконками в `MenuIconSlot`, стрелочная навигация включает их.
  Таймаут pick — 300s (диалог ждёт человека) зеркально в bg/messaging.
- Чистка: системный `PlayBadge` вместо самодельного, системная тень панели,
  скроллбар только у content preview по `[data-clipper-scrollbar]`,
  удалены `StatusBar.tsx` и мёртвый `PreviewCard.tsx`, тестбенч обновлён.
- Тесты: VaultSelect pinned-действия (4), SaveButton состояния (3),
  CollectionPicker Escape-очистка. SPEC_CLIPPER обновлена (layout, футер,
  протокол, Escape, состояния).

## 09.08.2026 — Переключатель пространств, групповое контекстное меню, сортировка коллекций

### Задача
Три UX-блока за одну сессию: закреплённые действия в переключателе пространств,
правый клик по групповому выделению, действующий на одну карточку вместо всех,
и грязное перетаскивание коллекций в сайдбаре — призрак строки, дёргающаяся
цель, пустая летящая копия, подвисания на быстрых движениях.

### Что сделано
- Переключатель пространств: закреплённые `Reveal in Finder` (текущее
  пространство, закрывает меню) и `Add space` с иконками в общем
  `MenuIconSlot`; строки пространств получили тот же слот пустым. Иконки строки
  (`FolderOpen`/`X`) несут tooltips; текст у крестика называет последствие:
  «Remove from the list — files stay on disk».
- Правый клик по карточке из выделения открывает selection-scoped меню
  (`GroupSelectionContextMenu`): count header, `Connect`, `Disconnect`,
  `Merge`, `Delete` — всё действует на выделение целиком. Одиночные команды
  (`Reveal in Finder`, `Copy Path`, `Rename…`, `Source`) отсутствуют, а не
  отключены. Общая логика с ⌘K-меню вынесена в
  `useGroupSelectionMenuActions`; подтверждение удаления — общий
  `DeleteSelectedCardsDialog`, смонтированный в Grid, а не в контенте меню.
- Сортировка коллекций переписана по контракту (DESIGN_SYSTEM → «Перетаскивание
  строк»): исходная строка — движущаяся невидимая дырка (transform идёт,
  `opacity-0`); копия в `DragOverlay` собрана из деталей настоящей строки —
  title cell, полоса миниатюр, счётчик; цель — слот под указателем
  (`pointerWithin` по сетке, замороженной на старте жеста, `closestCenter`
  только за пределами списка); порядок применяется оптимистично
  (`applyPendingTagOrder`), запись в vault догоняет; ховеры и превью на время
  жеста мертвы (`data-sidebar-tag-dragging` + `cursor: grabbing`); drop-полёт
  200ms в дырку. Подписка `useDndContext` изолирована от ядра сайдбара — при
  сортировке ядро не ререндерится вовсе.
- Жест закреплён браузерным аудитом `bun run test:sidebar-reorder`
  (роут `/__sidebar-reorder-audit`, 16 проверок: дырка/призрак, медиа в копии,
  точность слота под курсором, монотонность сдвигов, кадровые разрывы на
  быстром скрабе, мгновенный порядок, Escape). Включён в `test:browser`.

### Продолжение сессии: NSFV — фантомный канал, кнопка под видео, драг в compact-хроме
- Дубль коллекции «Видео» в NSFV: в таблице `channels` жила запись
  `Collections/Видео` — слаг с папкой из времён до идентификации коллекций по
  имени. Файловая уборка её не касалась (срабатывает на исчезновение файла, а
  этой записи файл никогда не отвечал). Хуже: её тег разрешался в документ
  настоящего «Видео», реордер планировал две записи в один путь, staged
  mutation отклонял всю транзакцию — поэтому в NSFV не сохранялось ни одно
  перетаскивание коллекций. Два фикса: sweep фантомных каналов в
  `reconcile_vault` (канал жив ⇔ его имя носит живой документ-канал из скана) и
  правило в `reorder_channels` — команда пишет только документ, чьё имя
  совпадает с тегом. Дубль имён впредь невозможен по построению: имена
  документов в `Collections/` уникальны файловой системой, а производная
  таблица теперь сверяется с ней при каждой полной реконсиляции.
- Кнопка «⋯» проваливалась под видео на feed-карточках: WKWebView поднимает
  `<video>` в собственный композитный слой, и позиционированный сосед с большим
  z-index всё равно рисовался под ним. Hover-слоям карточки выдан собственный
  слой (`transform-gpu`).
- Перетаскивание карточки «переставало работать» в compact-хроме Detail:
  заголовок там был `data-tauri-drag-region` (перетаскивание окна), а не block
  drag handle, как filename в classic-заголовке. Теперь заголовок — хэндл
  карточки в обоих режимах.

### Уроки
- `closestCenter` целится центром схваченного прямоугольника, а пользователь —
  остриём курсора: расхождение до полстроки читается как «дырка не там».
- Подписка на dnd-контекст в корне большого компонента превращает каждый сдвиг
  цели в полный ререндер; тонкая оболочка + memo-ядро оставляют движение
  строкам с их собственными `useSortable`-подписками.
- Метрики аудита тоже код: «без осцилляций» — это ноль смен направления, а не
  число промежуточных значений transition; ожидание слота не ставят на границу
  слотов.

## 04.08.2026 — Видео из постов X с возрастным ограничением

### Задача
В новом пространстве посты с видео сохранялись как текст без медиа. Причина
оказалась структурной: публичный syndication API отдаёт анонимному вызову
`TweetTombstone`, а в разметке страницы такое видео живёт под `blob:`-адресом,
действительным только внутри вкладки. Оба существующих пути извлечения
неприменимы по устройству, а не из-за ошибки.

### Что сделано
- Третий путь: расширение отдаёт нативному хосту ссылку на пост и куки своей
  сессии, хост вызывает `yt-dlp`. Авторизация не воспроизводится — куки берутся
  у браузера, где стоит расширение, поэтому вопрос выбора браузера снят. Решение
  делегировано намеренно: доставка видео у X — приватный интерфейс, который
  меняется по их расписанию, и отслеживание этих изменений составляет смысл
  существования `yt-dlp`.
- Запускается только когда первые два пути не дали видео. Для обычных постов не
  выполняется вовсе.
- Куки читаются только для доменов X, только для помеченных постов, пишутся во
  временный jar с правами `0600` и удаляются на всех путях выхода.
- Постер запрашивается тем же вызовом: без него превью откатывалось на
  `og:image` страницы, а там у ограниченного поста рекламная карточка X.

### Четыре причины, по которым это не работало
Каждая была найдена отдельным кругом, и первые три — гаданием:

1. Проверка срабатывания смотрела на количество найденных превью, а не на
   наличие ссылки. `blob:`-источник отбрасывается выше по потоку и оставляет
   запись с одним постером — ровно вид ограниченного поста.
2. Повтор стоял в момент сохранения, тогда как первый запрос жил в подготовке
   превью и возвращал статью нетронутой. Обе поверхности строились из пустого
   результата.
3. `yt-dlp` не находился: хост запускает браузер, а не оболочка, и он наследует
   `PATH` из четырёх системных каталогов, куда пакетные менеджеры не ставят.
4. Разрешённые ссылки не дописывались в тело заметки. Хост скачивает медиа,
   читая markdown; для обычных твитов ссылки складывает ветка публичного API, до
   которой ограниченный пост не доходит.

Первые три круга происходящее внутри браузера было невидимым: хост общается
через stdin/stdout, печать туда ломает протокол, stderr уходит в никуда. После
добавления файла лога каждая следующая причина находилась с первой попытки.

### Проверки
- `bun run verify:core` — passed: 750 frontend tests, Rust 656 passed /
  5 ignored, native host 58 passed, migration binaries 13.
- Живая проверка: пост с двумя роликами сохранён целиком — файлы 13 и 14 МБ,
  длительность 115 и 99 секунд, обе ссылки в теле в формате Obsidian.

### Заодно
- Диалог удаления больше не сообщает, что удалится «только элемент», когда у
  карточки нет медиа. Объяснения про медиа не тронуты.

## 02.08.2026 — Переключение каналов снова работает при открытой карточке

### Задача
Вернуть исходное поведение `Opt+Cmd+Up/Down`: при открытой карточке она
закрывается, а канал переключается.

### Что сделано
- Условие `selectedBlock` убрано из обработчика. Оно появилось не как
  продуктовое решение: в пакете исправлений после аудита шорткат заглушили сразу
  в четырёх контекстах одним правилом «не срабатывает в
  Detail/Search/overlay/editable». Для полей ввода, меню и списков это верно —
  там клавиши заняты чужой логикой. Detail попал туда за компанию: он
  полноэкранный просмотрщик внутри того же маршрута, а не модальное окно, и
  стрелки не перехватывает — это отмечено в той же записи аудита.
- Вместо `selectedBlock || isOverlayKeyboardTarget` используется
  `isDetailShortcutBlockedTarget`, который пропускает Detail по маркеру
  `data-detail-root` и продолжает блокировать остальные диалоги, меню, списки и
  overlay просмотра изображения.
- При срабатывании с открытой карточкой вызывается `handleDetailClose()` —
  обычный путь закрытия с анимацией и восстановлением фокуса в ленте.
- Тест, закреплявший блокировку, заменён на тест возвращённого поведения.

### Проверки
- `bun run verify:core` — passed: 79 frontend files / 721 tests, Rust 656 passed
  / 5 ignored, native host 56, migration binaries 13.

## 02.08.2026 — Верхняя полоса: накладка вместо маски, только тёмная тема

### Задача
Довести эффект верхней кромки до рабочего состояния: контент должен читаться как
уезжающий под chrome. Прошлые итерации давали то выцветание контента, то дёрганье
прокрутки, то полосу не на месте.

### Что сделано
- **Механика заменена.** Была маска прозрачности на контенте — она растворяет его
  в фон страницы, и на светлом фоне тёмная фотография выцветает в белый, читаясь
  как испорченная. Теперь это накладка: цвет поверхности кладётся поверх контента
  и растворяется вниз, контент проезжает под ней целым.
- **Эффект только в тёмной теме.** Накрытие почти чёрным затемняет контент — он
  уходит вглубь; накрытие почти белым осветляет, и фотография выглядит
  испорченной. Подбор под светлую тему каждый раз менял один дефект на другой,
  поэтому `isTopFadeSupported` пропускает только `dark`.
- **Цвет — фон поверхности, а не chrome.** Полоса, покрашенная `--chrome`,
  выглядела прямоугольником чужого оттенка: `oklch(0.17)` против `oklch(0.14)`.
  Теперь `--background` / `--sidebar` / `--card` по месту.
- **Полоса вынесена из прокручиваемого контейнера.** Внутри него `sticky` не
  выходит за padding box — в боковой панели полоса вставала на 32px ниже кромки и
  уже панели. Высота стала постоянной: вывод высоты из смещения прокрутки менял
  состояние на каждый пиксель и перерисовывал ленту раз в кадр, отчего прокрутка
  дёргалась. Меняется только `opacity`.
- **Единый контракт.** Одна высота 24px вместо двух профилей, один хук вместо
  двух источников сигнала (лента брала `scrollTop` из виртуализации, а он
  коалесцирован через RAF и отставал на кадр), обёртка боковой панели повторяет
  жёсткий `min-width` её `nav`.
- **Замеры референсов.** Профиль контраста по вертикали на обоих скриншотах
  (2x) дал полный контраст на 41–43 CSS-пикселях и одинаковые значения в обеих
  темах. Итоговая высота 24px — сознательное сокращение относительно замера:
  более высокая полоса съедала слишком много полезной площади.

### Проверки
- `bun run verify:core` — passed: 79 frontend files / 721 tests, Rust 656 passed
  / 5 ignored, native host 56, migration binaries 13.
- Feed desktop/narrow, Graph dark/light, cold-space first/settled/deep — без
  failures. Первый прогон Graph-гейта один раз упал по трёхсекундному timeout
  первой отрисовки, повторный прошёл; Graph в этой работе не затрагивался.
- Метрики прокрутки ленты после переделки: `maxFrameGapMs` 8.2 при бюджете 120.

### Статус
- Документация приведена в соответствие с реализацией: SPEC переписан,
  ARCHITECTURE решение 030, DESIGN_SYSTEM, PLAN Phase 31.

## 02.08.2026 — Верхнее затухание спроектировано заново

### Задача
Прежняя верхняя кромка была переносом кривой правой кромки: таблица из девяти
подобранных значений и ширина, выведенная из высоты текстовой строки. В ленте
контент — карточки с изображениями в сотни пикселей, никаких строк, поэтому
рампа в `12px` читалась как ореол вокруг кромки, а не как растворение.

### Что сделано
- Верхняя кромка отделена от правой: они решают разные задачи. Правая прячет
  переполнение текста и остаётся ручной таблицей с уходом в ноль; верхняя
  создаёт впечатление контента за границей и генерируется формулой.
- Кривая: `alpha(t) = minAlpha + (1 − minAlpha) · smootherstep(t)^1.8`.
  `smootherstep` даёт нулевые первую и вторую производные на обоих концах — иначе
  на стыках возникает полоса Маха. Степень `1.8` компенсирует гамму: браузер
  смешивает альфу линейно по sRGB, а восприятие яркости степенное.
- Длина привязана к масштабу контента, а не к типографике. Профиль
  `TOP_FADE_CANVAS` — `56px` и остаток `0.06` для ленты и развёрнутой карточки;
  `TOP_FADE_LIST` — `28px` и остаток `0.08` для боковой панели и поиска, где
  строки по 32px и длинная рампа съела бы строку целиком.
- Стопы генерируются из кривой примерно по одному на `2.5px` (12–24 штуки).
  Видимый артефакт — линейная интерполяция между стопами, а не 8-битная сетка.

### Проверки
- `bun run verify:core` — passed: 79 frontend files / 727 tests, Rust 656 passed
  / 5 ignored, native host 56, migration binaries 13.
- Feed desktop/narrow, Graph dark/light, cold-space first/settled/deep — без
  failures.

### Статус
- Значения профилей — расчётная стартовая точка, визуальная приёмка на реальном
  контенте не выполнялась.

## 02.08.2026 — Компактная верхняя кромка и починка растворения в поиске

### Задача
Сделать верхнее растворение вдвое компактнее, оставить у самой кромки слабый
остаток вместо полного исчезновения и починить поиск, где эффект не работал.

### Что сделано
- Ширины кромок разведены: `EDGE_FADE_WIDTH = 24` осталась у правой кромки
  боковой панели, верхняя получила собственную `TOP_FADE_WIDTH = 12`. Правая
  гасит целую строку текста вбок и нуждается в запасе на читаемость, верхней
  достаточно смягчить уход контента под chrome.
- Верхняя рампа больше не доходит до нуля: у кромки остаётся
  `TOP_FADE_MIN_ALPHA = 0.12`. Полностью растворённый контент читается как
  обрезанный, слабый остаток — как контент, продолжающийся за chrome. Форма
  кривой сохранена: значения перенесены из `[0, 1]` в `[0.12, 1]`.
- **Исправлен баг: в поиске растворение не работало вовсе.** `SearchOverlay`
  живёт внутри Radix `Dialog`, который не монтирует содержимое, пока окно
  закрыто. Список результатов появлялся в более позднем рендере, чем сам
  компонент, поэтому `useTopFadeMask` на момент выполнения эффекта видел пустой
  ref и больше не перезапускался. Это ровно тот инвариант, который был записан в
  спецификации прошлой сессией и не проверен на месте.
- Хук переведён с ref-объекта на callback ref и теперь возвращает
  `{ ref, style }`. Переданный `forwardRef` продолжает заполняться тем же узлом,
  поэтому боковая панель и Detail сохранили свою работу с этим ref. Инвариант о
  синхронном монтировании узла удалён — механизм больше его не требует.

### Проверки
- `bun run verify:core` — passed: bindings без drift, ESLint clean,
  79 frontend files / 717 tests, Rust main library 656 passed / 5 ignored,
...
- Новые тесты, которые поймали бы баг поиска: узел, появившийся в более позднем
  рендере; повторное подключение после закрытия и открытия диалога; три
  интеграционных теста на самом `SearchOverlay`.

### Статус
- Визуальная приёмка остатка `0.12` в собранном приложении не выполнялась;
  значение подбирается одной константой.

## 02.08.2026 — Растворение верхней кромки прокручиваемых поверхностей

### Задача
Убрать жёсткий срез контента у верхнего chrome: прокрученный контент должен
истаивать под меню. Включается настройкой, применяется к боковой панели, ленте,
развёрнутой карточке и поиску. Значения — те же, что у существующего затенения
превью в боковой панели.

### Что сделано
- Кривая затухания вынесена из `Sidebar.tsx` в общий `src/lib/edgeFade.ts`:
  `EDGE_FADE_WIDTH = 24`, девять пар `alpha/progress`, правая и верхняя кромки
  строятся из одного набора. Боковая панель переведена на общий модуль, второй
  копии кривой не осталось.
- Эффект реализован маской прозрачности, а не тенью. Тёмный градиент требует
  разных значений на тему и противоречит `DESIGN_SYSTEM.md`, где `box-shadow`
  зарезервирован за всплывающими элементами, а планы разделяются уровнями фона,
  границами и масками.
- Активация разделена по владельцу смещения: `Grid` уже держит собственный
  `scrollTop` для виртуализации и резолвит маску через `topFadeMaskStyleFor`, не
  добавляя второй слушатель в горячий путь ленты; боковая панель, Detail и поиск
  используют `useTopFadeMask`, который хранит одно булево, а не смещение.
- В покое маска выключена: порог `scrollTop >= 1px` отсекает субпиксельные
  смещения инерционной прокрутки, первая строка остаётся непрозрачной.
- Настройка `mine.scrollEdgeFade` выключена по умолчанию, строка Appearance
  `Fade content under the chrome`, синхронизация через существующий
  `settings-changed`.
- `FeedScrollAuditRoute` теперь читает ту же настройку: без этого гейт
  производительности ленты физически не мог измерить ленту с активной маской.

### Проверки
- `bun run verify:core` — passed: bindings без drift, ESLint clean,
  79 frontend files / 709 tests, Rust main library 656 passed / 5 ignored,
  native host 56 passed, migration binaries 13 passed.
- Feed desktop/narrow, Graph dark/light и cold-space first/settled/deep browser
  gates прошли без failures.
- Отдельный замер прокрутки ленты с маской и без (headless Chromium, 60 шагов до
  конца ленты): p95 кадра 25.7 мс против 25.8 мс, максимум 49.9 мс против
  50.0 мс, среднее 15.84 мс против 15.62 мс. Разница в пределах шума — маска на
  скроллпорте прокрутку не ухудшает.

### Статус
- Замер выполнялся в headless Chromium, не в WKWebView; визуальная приёмка в
  собранном приложении не выполнялась.
- Ветка `worktree-scroll-edge-fade` от `audit-remediation`.
...
### Проблема
В правом верхнем углу Search Overlay при вводе запроса мог появляться счётчик
вроде `508`, даже когда список результатов был пустым. Это выглядело как
количество найденных карточек, но фактически было `GridSnapshot.total_blocks` —
общий non-channel размер vault для `Everything`.

### Причина
`list_grid_blocks` всегда заполняет `total_blocks` через `count_grid_blocks()`.
Для обычной ленты это нужно сайдбару, но для overlay search это не search-hit
count. Дополнительно header показывал старое значение сразу после смены query,
пока новый debounced-запрос ещё не завершился.

### Что сделано
- Search Overlay теперь показывает количество реально отрисованных строк
  текущей выдачи: `0`, `N` или `N+`, если backend вернул `has_more`.
- Счётчик привязан к завершённому запросу текущей строки ввода и не показывает
  stale recent/vault total во время debounce/in-flight состояния.
- `SPEC_SEARCH_OVERLAY.md` обновлён: `total_blocks` явно не используется для
  header count.

### Тесты
- `SearchOverlay.test.tsx`: добавлены регрессии на старый `508`, `0 results`
  и `N+` при `has_more`.
