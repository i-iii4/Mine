...
`ChromePlate` owns its centered 24px visual surface. Dropdowns anchor to the
full-height target, so their default 4px offset leaves 3px beyond the divider.
Reusable actions and segmented controls opt in through `chrome`; feed and
clipper geometry do not inherit this behavior. Contract: `DESIGN_SYSTEM.md`.
...
## Обновление готовых превью

`thumb:updated` инвалидирует строку карточки независимо от её типа. App передаёт
загруженные slug в `previewRowQueue`: дедупликация, окно 32 ms, пакеты до 200,
один запрос одновременно. `get_grid_rows` читает общий feed read model внутри
SQLite snapshot и возвращает `GridRowsSnapshot { path, generation, blocks }`.
Нет повторного сканирования файлов или загрузки всего диапазона ленты.

App заменяет только существующие строки с сохранением порядка и object identity.
Ревизии строк не позволяют позднему полному snapshot откатить новое превью;
при этом частичный ответ не продвигает ревизию всего маршрута и не блокирует
обновление его состава. Очередь отменяется при смене маршрута/пространства.
Preview-only `vault-changed` сохраняется для других подписчиков, но не запускает
полную перезагрузку основной ленты.
...
│   └── file-identity.json           ← вспомогательная история переименований
...
Согласованный [контракт совместимости с Obsidian](</Users/i_iii/Проекты/Личные проекты/local-arena/docs/space-storage-requirements.md#совместимость-с-obsidian>) разделяет три роли: файлы содержат действующие связи, каталог путей позволяет их разрешить, история помогает восстановить отсутствующее старое имя. История не переопределяет действующую ссылку.

[Общее ядро ссылок](</Users/i_iii/Проекты/Личные проекты/local-arena/mine-core/src/links.rs>) принимает снимок путей через `LinkIndex::new`. `resolve(source_path, target, syntax)` возвращает `Resolved`, `Missing` или `Ambiguous`. `shortest_link` формирует кратчайшую ссылку, которая разрешается обратно в выбранную цель. Нативный код и WASM используют этот модуль, без отдельных правил в JavaScript.

Obsidian-ссылки сначала проверяются как существующий адрес от корня, затем как однозначный суффикс. Устаревший путь восстанавливается по имени только при единственной цели. Обычные Markdown-ссылки разрешаются относительно исходного документа. Каталог строится один раз на пакет обработки. Новые имена выбираются во всём пространстве, существующие одноимённые файлы не объединяются.

Вспомогательная история в `.mine/file-identity.json` хранит прежний путь, физический ключ и ранее наблюдавшуюся ссылку. Она применяется только после неуспешного разрешения действующей ссылки. Подтверждённое новое имя записывается в сам Markdown. Отсутствие истории не мешает чтению и перестановке файлов с уникальными именами; произвольное внешнее переименование без истории восстановить нельзя.

Сохранение карточки не зависит от успешной записи истории. Публикация нового файла не перезаписывает существующий, повтор операции сверяется с журналом сохранения. Нативный адаптер повторно проверяет занятость имени перед публикацией.

Исправление существующих ссылок сохраняет посторонние свойства и текст. На macOS атомарный обмен позволяет проверить фактически вытесненную версию при конкурирующем сохранении заменой файла. Конфликтная версия сохраняется для восстановления, а исправление не подтверждается как успешное. Это не межприложенческая блокировка и не обещание защиты от любых записей через давно открытый файловый дескриптор.

Подробные алгоритмы и критерии проверки: [спецификация идентичности](</Users/i_iii/Проекты/Личные проекты/local-arena/SPEC_IDENTITY_ROBUSTNESS.md#архитектура-нового-механизма>). Статус приёмки фиксируется отдельно в [плане](</Users/i_iii/Проекты/Личные проекты/local-arena/PLAN.md#совместимость-с-obsidian-и-вспомогательная-история-переименований>).
...
### 043: Доступ AI к материалам — ядро, затем CLI, затем MCP

Интерфейс для AI-агентов даёт производные данные (гибридный поиск, коллекции,
резолв медиа) и при этом самодостаточен: сырое содержимое отдаётся через него
же, потому что у MCP-потребителя без шелла другого канала нет. Читающий API выносится в ядро; первым слоем — CLI `mine` (дёшево,
отлаживаемо, полезно человеку), MCP-сервер — тонкий адаптер над тем же ядром,
когда появится потребитель без шелла. JSON-контракт CLI проектируется как
будущие схемы MCP-инструментов, чтобы второй слой не потребовал второго
контракта. Реализовано целиком 27.08.2026: ядро, CLI (чтение, полевые и
структурные мутации, установка `bun run cli:install`) и MCP-адаптер
`mine mcp` (`mcp.rs`) — stdio JSON-RPC без внешних зависимостей, 19
инструментов транслируются в argv и проходят через `cli::run`, так что
поведение совпадает с CLI по построению. Требования —
[SPEC_AI_ACCESS.md](SPEC_AI_ACCESS.md).

### 044: Ни типов, ни классов карточек — одно решение об отображении

Принято 27.08.2026 (уточнено в тот же день). Упраздняются и поле `type`, и понятия
«статья»/«картинка» как классы. Карточка в ленте либо показывает только
изображение, либо выводит дополнительную информацию, и решает это один
параметр: содержимое тела за вычетом медиа-вставок. Пусто — только
изображение; непусто — информация; медиа нет — текст. Поле `type` не пишется
