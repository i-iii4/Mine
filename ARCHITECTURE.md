...
контракта. Требования — [SPEC_AI_ACCESS.md](SPEC_AI_ACCESS.md).

### 044: Вид карточки выводится из содержимого; поле `type` упраздняется

Принято 27.08.2026. Источник истины — содержимое: медиа с короткой подписью —
медиа-карточка, длинный текст — статья, голый `url` — ссылка. Поле `type` не
пишется и не читается для карточек; единственное живое значение —
`type: channel`, маркер страницы коллекции (пустую коллекцию в плоском
хранилище иначе не распознать). Существующие файлы вычищаются одной
перезаписью — хранилище личное, церемония миграции избыточна. Обоснование и
целевой контракт — [SPEC_BLOCK.md](SPEC_BLOCK.md). Реализация не начата.
