...
### 041: Размер узла графа считается от заполнения кадра

Узел графа занимал ровно 32 экранных пикселя при любом масштабе: приближение
раздвигало связи, но не показывало содержимое крупнее.

Первая реализация привязала размер к масштабу — и это не сработало, что
выяснилось в тот же день. Расстояния между узлами умножаются на тот же
масштаб, поэтому карточка и промежуток рядом с ней росли одинаково: доля
экрана под картинками не менялась вообще. Приближение ощущалось как
бесконечное расхождение, а потолок доставался только тогда, когда в кадре
оставалась одна карточка.

Размер — функция **масштаба и плотности раскладки**, и это третья модель за
день. Вторая считала от числа узлов в кадре: она давала верную пропорцию, но
вход был дискретным — один узел вышел за край, и размер прыгнул у всех.
Сглаживание, которым эти ступени скрывались, стало запаздыванием за трекпадом.

Число узлов в кадре есть плотность, умноженная на площадь кадра; площадь кадра
есть окно, делённое на квадрат масштаба. Подстановка даёт размер, прямо
пропорциональный масштабу с коэффициентом от плотности. Вход непрерывен, жест
отображается точно, а плотность — то немногое, что действительно стоит
сглаживать, потому что меняется без участия руки.

Отсюда общее правило: **прямое манипулирование не анимируют**. Анимация
уместна там, где изменение происходит само, и становится запаздыванием там, где
ведёт рука.

Потолок выведен из раскладки: коллизия держит центры карточек не ближе 44
единиц графа, значит на экране между ними `44 × zoom` пикселей, и рост картинки
до этой величины означает касание. 100 пикселей оставляют пятую часть на
воздух. Ограничение при этом обязано считаться по фактическому минимальному
расстоянию в текущей раскладке, а не по номинальному радиусу: коллизия мягкая,
и на плотных участках соседи стоят ближе расчёта.

Предел приближения — обращение той же функции, а не отдельный расчёт:
`zoomForNodeSize(потолок, плотность)`. Две формулы для связанных величин
расходятся при первой же правке одной из них, и приближение начинает
останавливаться там, где карточки перестали расти вчера. Константа здесь была
бы произвольной и оказалась вчетверо велика при первой же проверке.

Отвергнута альтернатива, где после порога камера замирает и зум переключается
на рост картинок: она требует перехватывать жест и возвращать камеру, то есть
бороться с собственным обработчиком зума — механизмом, который уже дважды
порождал рывки в графе.

Предусловие работы: свести размер узла к одному источнику. Сегодня константу
читают четыре независимых места — отрисовка, попадание курсора, позиция меню и
позиция предпросмотра. При постоянном размере разъехаться они не могли; при
переменном любое расхождение означает курсор, попадающий мимо того, что видит
глаз.
