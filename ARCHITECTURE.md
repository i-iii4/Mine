...
Размер считается от того, **сколько узлов сейчас в кадре**: площадь кадра
делится между ними, и от доли каждого берётся `GRAPH_NODE_FILL_RATIO`.
Величина доли выбрана не на вкус — её задаёт нижняя граница в 32 пикселя: при
0.55 полный обзор давал расчётные 24, то есть карточки лежали на границе всю
первую половину приближения и не менялись.
...
Из того же расчёта выводится и предел приближения: приближать осмысленно ровно
до момента, когда размер упёрся в потолок. Константа здесь была бы произвольной
и оказалась вчетверо велика при первой же проверке.
