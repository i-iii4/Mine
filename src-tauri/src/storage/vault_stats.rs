...
pub fn scan_source_vault_file_stats(vault: &VaultLayout) -> Result<SourceVaultFileStats> {
    ...
    scan_source_vault_file_stats_inner(vault.root(), &mut stats)
        .with_context(|| format!("failed to scan source vault stats: {}", vault.root().display()))?;
    Ok(stats)
}

fn scan_source_vault_file_stats_inner(dir: &Path, stats: &mut SourceVaultFileStats) -> Result<()> {
    for entry in std::fs::read_dir(dir)? {
        ...
        if file_type.is_dir() {
            ...
            scan_source_vault_file_stats_inner(&path, stats)?;
            continue;
        }

        if hidden_file_name(&path) {
            continue;
        }
...
mod tests {
    ...
    fn vault_stats_count_everything_and_current_collection_from_index() {
        ...
        insert_block(&conn, &make_block("alpha", BlockType::Article, "body", &["Design"]), &vault);
        insert_block(&conn, &make_block("beta", BlockType::Image, "", &["Other"]), &vault);
        insert_block(&conn, &make_block("Design", BlockType::Channel, "", &[]), &vault);
