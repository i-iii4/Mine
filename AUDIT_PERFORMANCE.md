...
### 20.08.2026 — Тормоза оказались нехваткой памяти, а не приложением

Жалоба на подтормаживание и на «вчера работало идеально». Проверено по
процессам, а не по ощущениям, и приложение оказалось почти ни при чём.

Что показали замеры. Профиль веб-процесса во время перетаскивания ширины
панели: все потоки в ожидании, реальной работы 0,7% и та на подгрузку медиа.
Профиль процесса композиции: девять замеров работы из 3817. Загрузка
видеокарты 11–25%. Браузерный замер изменения ширины: 8,3 мс на кадр, что
равно интервалу при 120 Гц, ни одного пропуска — и та же цифра при полностью
удалённых из панели строках.

Что нашлось на самом деле: **свободной памяти ноль из 24 ГБ, подкачка занята
на 6,5 из 8 ГБ**, 3,2 млн выгрузок и 2,3 млн загрузок страниц. Любое
приложение в таком состоянии ждёт диск вместо памяти — при свободном
процессоре и свободной видеокарте, ровно как показывали профили. После
освобождения памяти пользователь подтвердил, что приложение поехало.

Крупнейшие потребители на момент замера: сессии агента 1,65 ГБ, веб-часть
приложения 613 МБ, ChatGPT 557 МБ, приложение Claude 502 МБ, процесс
композиции 320 МБ, сам Mine 231 МБ.

Наш вклад — около 900 МБ на веб-часть и композицию при 571 карточке. Это
декодированные превью; уменьшать их стоит, но это работа про память, а не про
скорость отрисовки.

Ложные версии, отброшенные по дороге: пересчёт раскладки (0,014 мс), пересчёт
высот (0,55 мс), маски затухания в панели (проба со снятыми масками не дала
разницы), миниатюры панели (удаление всех строк в браузерном замере не дало
разницы).


### 20.08.2026 — Слои композиции: стёкла только под видео

Приложение подтормаживало при неподвижном окне. Замер процессов показал, где
уходит работа: слой отрисовки держал **30–36% процессора постоянно**, а суммарно
приложение занимало **1,3 ГБ**. Ни JavaScript, ни Rust при этом не работали —
профилирование обоих процессов показало потоки в ожидании.

Причина — `transform-gpu` на трёх слоях `CardHoverMenu`: затемнение, кнопка
меню и нижний ряд действий. Он поднимал каждый слой в собственный слой
композиции с отдельным буфером в видеопамяти, и делал это **на каждой карточке
в окне**, а не под курсором. Десятки карточек — сотни буферов.

Проверено экспериментом, а не рассуждением: сборка со снятыми слоями дала
**320 МБ и 5–9%** против 1,3 ГБ и трети ядра.

Удалять насовсем нельзя — слои появились из-за реального дефекта: WKWebView
поднимает `<video>` в собственный слой, и кнопки проваливались под ролик,
несмотря на `z-index`. Поэтому слой теперь просят только карточки, под которыми
играет видео (`videoUnderneath`). После правки в покое: **0,3–1,5%** на
отрисовку, 245 МБ у приложения.

Осталось открытым: веб-контент держит **586 МБ** на ленте из 571 карточки —
это декодированные превью, отдельная задача.
